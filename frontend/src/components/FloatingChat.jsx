...
export default function FloatingChat({ userEmail, userRole }) {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
            {isOpen && (
                <div className="relative mb-4 w-[380px] sm:w-[450px] bg-slate-900 border border-slate-700 shadow-2xl rounded-2xl overflow-hidden flex flex-col shadow-blue-900/20 shadow-[0_0_50px_rgba(37,99,235,0.3)] animate-in slide-in-from-bottom-5 duration-300">
                    <div className="bg-slate-800 p-3 flex justify-between items-center border-b border-slate-700 shrink-0">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-slate-100 text-sm tracking-wide">Nestlé Global Assistant</span>
                        </div>
                    ...
                    {/* Inner wrapper to override DisputeChat's fixed height slightly and hide its top border radius */}
                    <div className="relative w-full [&>div]:h-[550px] [&>div]:border-0 [&>div]:rounded-none [&>div]:shadow-none">
                        <DisputeChat
                            referenceNumber={userEmail ? `GLOBAL-${userEmail}` : `GLOBAL-${userRole}`}
                            userRole={userRole}
                            userEmail={userEmail}
                            contextData={{ status: 'Active Workspace', type: 'General Support' }}
                        />
                    </div>
                </div>
            )}
            ...
            <button
                ...
                title={isOpen ? "Close Assistant" : "Need help? Chat with AI or Live Support"}
            >
