                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                                        ...
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="text-right">
                                                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ring-1 ring-inset ${log.status?.includes('Approve') || log.status?.includes('Generated') || log.status === 'Downloaded' ? 'bg-emerald-900/50 text-emerald-400 ring-emerald-500/20' :
                                                                        log.status?.includes('Reject') || log.status?.includes('Discrepancy') ? 'bg-red-900/50 text-red-400 ring-red-500/20' : 'bg-amber-900/50 text-amber-400 ring-amber-500/20'
                                                                    }`}>{log.status}</span>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-slate-800 rounded-xl p-4 text-center">
                                <p className="text-xs text-slate-400">💡 Pro tip: Upload BOQ to get official PO within 24h.</p>
                            </div>
                        </div>
                    </div>

                    {error && (
                        <div className="fixed bottom-5 right-5 bg-red-900/80 border border-red-700 text-red-200 px-4 py-2 rounded-lg shadow-lg text-sm z-50 backdrop-blur-sm">
                            ⚠️ {error}
                            <button onClick={() => setError(null)} className="ml-3 text-red-300">✖</button>
                        </div>
                    )}
                    {dbStatus && (
                        <div className="fixed bottom-5 left-5 bg-emerald-900/80 border border-emerald-700 text-emerald-200 px-4 py-2 rounded-lg shadow-lg text-sm z-50 backdrop-blur-sm">
                            ✅ {dbStatus}
                        </div>
                    )}
                </div>
