...
import { Send, Paperclip, MessageSquareWarning, RefreshCw, Clock, CheckCircle2, Download, Tag } from 'lucide-react';
...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit }) {
    ...
    const sendMessage = async (text = newMessage) => {
        ...
        try {
            await axios.post('https://nestle-finance-command-production.up.railway.app/api/sprint2/disputes/send', {
                referenceNumber, senderEmail: userEmail, senderRole: userRole, message: text
            });
            setNewMessage('');
            fetchMessages();
    ...
    return (
        <div className="flex flex-col h-[550px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl overflow-hidden font-sans">
            <div className="px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center relative overflow-hidden">
                <div className={`absolute top-0 left-0 w-full h-1 ${disputeStatus === 'Open' ? 'bg-amber-500' : 'bg-emerald-500'}`}></div>
                <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${disputeStatus === 'Open' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/40 dark:text-amber-400' : 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/40 dark:text-emerald-400'}`}>
