        ...
        <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            <div className="md:w-64 bg-slate-900 dark:bg-slate-900 text-slate-300 flex flex-col shadow-2xl z-10 shrink-0">
                ...
                <div className="grow py-6 flex flex-col gap-2 px-4 overflow-y-auto">
                    ...
                    <button onClick={() => setActiveTab('warehouse')} className={`text-left px-4 py-3 rounded-xl font-medium transition-all ${activeTab === 'warehouse' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'}`}>
                        📦 Warehouse (GRN)
                    </button>
            ...
            <div className="grow overflow-y-auto p-4 md:p-8">
                ...
                {activeTab === 'warehouse' && <WarehousePortal user={user} />}
...
function FinancePortal({ user }) {
    ...
    const filteredRecords = records.filter(r => {
        ...
        if (filter === 'Pending') return r.match_status.includes('Reject') || r.match_status.includes('Discrepancy');
                    ...
                    <table className="w-full text-left text-sm min-w-[1100px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredRecords.map((r) => {
                                ...
                                // PO upload time: from related BOQ created_at
                                const poUploadTime = relatedBoq ? new Date(relatedBoq.created_at) : null;
                                // Invoice upload time: reconciliation processed_at
...
function AnalyticsPortal() {
    ...
    // Fetch full reconciliation data for charts
    useEffect(() => {
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                        ...
                        {/* Bar Chart: Monthly Document Count */}
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
                        ...
                        {/* Bar Chart: Top Vendors by Value */}
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
