                                                                            ...
                                                                            <div className="relative pl-6 space-y-4 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
                                                                                {myTimeline.events.map((event, idx) => (
                                                                                    <div key={idx} className="relative">
                                                                                        <div className={`absolute -left-[21px] top-1 w-5 h-5 rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center text-[10px] z-10 ${event.status === 'completed' ? 'bg-emerald-500 text-white' :
                                                                                        ...
                                                                                        <div className="flex justify-between items-start">
                                                                                            ...
                                                                                            <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-tighter ${event.status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
...
function PayoutCalendar({ user }) {
    ...
    const markPaid = async (payout) => {
        if (!window.confirm(`Pay ${formatCurrency(payout.final_amount || payout.base_amount)} to ${payout.supplier_email}?`)) return;
        try {
            ...
            alert('Funds paid successfully.');
            fetchPayouts();
        } catch (error) {
            alert('Failed to pay funds.');
        }
    };
    ...
    const handleBatchAndPay = async (batch) => {
        ...
        try {
            ...
            alert('Batch payment successful. Funds paid.');
            fetchPayouts();
        } catch (error) {
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-32">
            {/* Header Area */}
                                        ...
                                        <tbody className="divide-y divide-slate-800/50">
                                            {upcoming.map(p => (
                                                <tr key={p.id} onClick={() => setSelectedPayoutId(p.id)} className="hover:bg-slate-800/30 transition-all cursor-pointer group">
                                                    ...
                                                    <td className="p-6">
                                                        <div className="flex items-center gap-2">
                                                            <span className={`w-2 h-2 rounded-full ${p.status === 'Paid' ? 'bg-emerald-400' :
                                                                    p.status === 'Scheduled' ? 'bg-blue-400' :
                                                                        p.status === 'Hold' ? 'bg-amber-400' :
                                                                            p.status === 'Renegotiated' ? 'bg-purple-400' :
                                                                                'bg-slate-400'
                                                                }`}></span>
                                                            <span className={`text-[10px] font-black uppercase tracking-wider ${p.status === 'Renegotiated' ? 'text-purple-400' :
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {topUpRequests.map(req => (
                                    <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                        ...
                                        <td className="p-6">
                                            <span className={`px-2.5 py-1 text-[10px] font-black uppercase rounded-full ${req.status === 'Approved' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' :
