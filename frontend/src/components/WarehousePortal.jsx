            ...
            <div className="flex-1 px-3 py-4 sm:px-8 sm:py-8 max-w-7xl mx-auto w-full pb-28 sm:pb-8">
                ...
                {loading && pos.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        <div className="flex p-1.5 bg-slate-200/50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 w-full sm:w-fit shadow-inner">
                            <button
                                onClick={() => { setViewMode('pending'); setSearchTerm(''); }}
                                className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-sm font-black transition-all ${viewMode === 'pending' ? 'bg-white dark:bg-slate-800 shadow-md text-blue-600 dark:text-blue-400 ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                            >
                                ⏳ Pending ({pendingList.length})
                            </button>
                            <button
                                onClick={() => { setViewMode('completed'); setSearchTerm(''); }}
                                className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl text-sm font-black transition-all flex items-center justify-center gap-1.5 ${viewMode === 'completed' ? 'bg-white dark:bg-slate-800 shadow-md text-emerald-600 dark:text-emerald-400 ring-1 ring-slate-200 dark:ring-slate-700' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                            >
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden relative">
                        <div className="lg:w-1/3 bg-slate-50 dark:bg-slate-800/30 p-4 sm:p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 flex flex-col">
                            ...
                            {/* Mindiee OCR Schema Fields Section */}
                            <div className="mb-6 bg-white dark:bg-slate-900/40 rounded-xl border border-slate-200 dark:border-slate-800 p-4 shadow-sm">
                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                                    <Bot className="w-3 h-3" /> Document Intelligence
                                </h4>
