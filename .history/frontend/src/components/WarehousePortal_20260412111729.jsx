...
export default function WarehousePortal({ user, onLogout }) {
    ...
    const syncPendingGRNs = async () => {
        ...
        const queue = [...syncQueue]; // Use current queue snapshot
        ...
        fetchPOs(); // Refresh list to reflect synced GRNs
    };
    ...
    const submitGRN = async () => {
        ...
        if (isOffline) {
            ...
            try {
                ...
                // Fallback: save offline if online submission fails
                const newQueue = [...syncQueue, payload];
                setSyncQueue(newQueue);
                setSelectedPO(null);
                setViewMode('completed');
            ...
            <div className="flex-1 px-3 py-4 sm:px-8 sm:py-8 max-w-7xl mx-auto w-full pb-28 sm:pb-8">
                ...
                {loading ? (
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden relative">
                        ...
                        <div className="lg:hidden sticky bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 pb-safe z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                            <button
                                onClick={submitGRN}
