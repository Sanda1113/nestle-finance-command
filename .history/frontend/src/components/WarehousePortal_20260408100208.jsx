...
import { Package, Truck, CheckCircle2, AlertCircle, ScanBarcode, ArrowLeft, Camera, Search, EyeOff, Eye, CalendarDays, Hash, ShieldAlert, ShieldCheck, WifiOff, MapPin, RefreshCw, LogOut, Moon, Sun } from 'lucide-react';
...
export default function WarehousePortal({ user, onLogout }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300">
            {/* Top Navigation for Standalone App */}
            <div className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800 p-4 px-6 flex justify-between items-center sticky top-0 z-20 shadow-sm shrink-0">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 className="text-xl font-extrabold tracking-tight text-white">Nestle<span className="text-blue-500">Warehouse</span></h1>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Logistics Portal</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    {/* Status Indicators */}
                    {isOffline ? (
                        <span className="flex items-center gap-1.5 bg-red-500/20 text-red-400 border border-red-500/30 px-3 py-1 rounded-full text-xs font-bold animate-pulse">
                            <WifiOff className="w-3 h-3" /> Offline (Local Storage)
                        </span>
                    ) : (
                        <span className="hidden sm:flex items-center gap-1.5 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-3 py-1 rounded-full text-xs font-bold">
                            <CheckCircle2 className="w-3 h-3" /> Online & Synced
                        </span>
                    )}
                    ...
                    {/* Dark Mode Toggle */}
                    <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 text-slate-300 hover:bg-slate-800 rounded-lg transition-colors">
                        {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                    </button>

                    {/* Logout */}
                    {onLogout && (
                        <button onClick={onLogout} className="flex items-center gap-2 px-3 py-2 bg-red-900/40 hover:bg-red-600 text-white rounded-lg text-sm font-bold transition-colors">
                            <LogOut className="w-4 h-4" /> <span className="hidden sm:block">Exit</span>
                        </button>
                    )}
                </div>
            </div>
            ...
            <div className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full">
                ...
                {/* Sync Alerts */}
                {isSyncing && (
                    <div className="mb-6 bg-blue-500 text-white p-3 rounded-xl flex items-center justify-center gap-3 font-bold shadow-lg">
                        <RefreshCw className="w-5 h-5 animate-spin" /> Connection Restored. Syncing offline data...
                    </div>
                )}
                {syncQueue.length > 0 && !isOffline && !isSyncing && (
                    <button onClick={syncPendingGRNs} className="mb-6 w-full bg-amber-500 hover:bg-amber-600 text-white p-3 rounded-xl flex items-center justify-center gap-3 font-bold shadow-lg transition-colors">
                        <RefreshCw className="w-5 h-5" /> You have {syncQueue.length} pending GRNs. Click to force sync to Finance.
                    </button>
                )}
                ...
                {loading ? (
                    ...
                    <div className="space-y-6">
                        <div className="flex gap-3 max-w-2xl">
                            ...
                            <button onClick={() => setScanning(!scanning)} className={`px-4 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all ${scanning ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-800 dark:bg-slate-800 text-white hover:bg-slate-700'}`}>
                                <ScanBarcode className="w-5 h-5" /> <span className="hidden sm:block">{scanning ? 'Stop Scan' : 'Hardware Scan'}</span>
                            </button>
                        </div>
