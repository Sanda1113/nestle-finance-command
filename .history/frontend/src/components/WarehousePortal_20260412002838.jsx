...
import { Package, Truck, CheckCircle2, AlertCircle, ScanBarcode, ArrowLeft, Camera, Search, EyeOff, Eye, CalendarDays, Hash, ShieldAlert, ShieldCheck, WifiOff, MapPin, RefreshCw, LogOut, Moon, Sun, X, Bot, Info, ArchiveRestore, Keyboard } from 'lucide-react';
...
const BarcodeScannerUI = ({ onScanSuccess, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (showManual) return; // Don't init camera in manual mode
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm flex items-end justify-center animate-in fade-in duration-200" onClick={onClose}>
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-3xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300" onClick={e => e.stopPropagation()}>
                {/* Header */}
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <ScanBarcode className="w-5 h-5 text-blue-500" />
                        <h3 className="font-black text-slate-800 dark:text-white">Scan Barcode</h3>
                    </div>
                    <div className="flex gap-2">
                        {!showManual && (
                            <button onClick={switchToManual} className="p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Manual Entry">
                                <Keyboard className="w-4 h-4" />
                            </button>
                        )}
                        {showManual && (
                            <button onClick={switchToCamera} className="p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Use Camera">
                ...
                <div className="p-5">
                    {showManual ? (
                        <form onSubmit={handleManualSubmit} className="space-y-4">
                            <p className="text-sm text-slate-500 dark:text-slate-400">Enter barcode or PO number manually:</p>
                            <input
                                type="text"
                                value={manualInput}
                                onChange={e => setManualInput(e.target.value)}
                                placeholder="e.g. SHP-12345 or 890123456789"
                                className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-blue-500"
                                autoFocus
                            />
                            <div className="flex gap-2">
                                ...
                                <button type="button" onClick={onClose} className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-xl transition-colors">
                                    Cancel
                                </button>
                            </div>
                        </form>
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-slate-600 dark:text-slate-300 font-medium">Camera not available</p>
                            <p className="text-sm text-slate-500 mt-1">Please grant camera permission or use manual entry.</p>
                            <button onClick={switchToManual} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold">
                                Enter Manually
                            </button>
                        </div>
                    ) : (
