...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const fetchPOs = async () => {
        try {
            ...
            const sorted = (res.data.data || []).sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
            setMyPOs(sorted);
    ...
    // Group logs by Deal/Reference Number for Timeline
    const groupedLogs = useMemo(() => {
        const groups = {};
        myLogs.forEach(log => {
            const ref = log.ref || 'General Activity';
            ...
            <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
                ...
                <div className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800 p-4 px-6 flex flex-wrap justify-between items-center sticky top-0 z-20 shadow-sm">
                    ...
                    <div className="flex items-center gap-4">
                        <button type="button" onClick={() => { fetchPOs(); fetchLogs(); }} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors" title="Force Refresh Data">
                            ...
                            {mode === 'logs' && (
                                <div className="animate-in fade-in duration-300">
                                    <div className="mb-4">
                                        ...
                                        <p className="text-sm text-slate-400">Activity grouped by Deal Reference.</p>
                                    </div>
                                    ...
                                    {Object.keys(groupedLogs).length === 0 ? (
                                        ...
                                        Object.entries(groupedLogs).map(([refId, logsForDeal]) => (
                                            <div key={refId} className="bg-slate-900 rounded-xl shadow-sm border border-slate-800 overflow-hidden mb-6">
                                                <div className="bg-slate-800/80 px-4 py-3 border-b border-slate-700 flex justify-between items-center">
                                                    <h3 className="font-bold text-slate-200 flex items-center gap-2">
                                                        <Tag className="w-4 h-4 text-blue-400" /> Deal Lifecycle: {refId}
                                                    </h3>
                                                </div>
                                                <div className="p-5">
                                                    <div className="relative border-l-2 border-slate-800 ml-3 space-y-6 pb-2">
                                                        {logsForDeal.map((log) => {
                                                            ...
                                                            // 🛡️ ENHANCED DYNAMIC STATUS OVERRIDE
                                                            if (log.type.includes('Match') || log.type.includes('Reconciliation')) {
                                                                ...
                                                                // Always respect math discrepancy
                                                                if (liveStatus?.includes('Discrepancy') || liveStatus?.includes('Manual Review')) {
                                                                    liveStatus = 'Discrepancy Detected';
                                                                } else if (liveStatus === 'Pending' || liveStatus === 'Processing' || !liveStatus || liveStatus.includes('Matched')) {
                                                                    liveStatus = 'Matched - Pending Finance Review';
                                                                } else if (liveStatus.includes('Approve')) {
                                                                ...
                                                                } else if (liveStatus.includes('Reject')) {
                                                                    liveStatus = 'Rejected by Finance';
                                                                }
                                                            }
