...
export default function WarehousePortal({ user, onLogout }) {
    ...
    const processScanResult = (decodedText) => {
        ...
        // 🔹 SCENARIO B: Shipment-Level Scanning (Match by Shipment ID now, not PO Number)
        if (!state.selectedPO) {
            // Check if the barcode matches either the PO Number OR the new Shipment ID
            const matchedPO = state.pos.find(p =>
                p.po_number.toLowerCase() === decodedText.toLowerCase() ||
                p.shipment_id?.toLowerCase() === decodedText.toLowerCase()
            );

            if (matchedPO) {
                if (matchedPO.status && matchedPO.status.includes('Received')) {
                    alert(`📦 Shipment ${matchedPO.shipment_id} found, but it has already been COMPLETED and locked.`);
                } else {
        ...
        if (matchIndex !== -1) {
            ...
            setDetectedProduct({
                ...
                name: "Unknown Scanned Item",
                category: "Unrecognized Inventory",
                message: `❌ Item not in shipment. This item does not match any expected items for Shipment ${state.selectedPO.shipment_id}. Please segregate.`
            });
    ...
    const pendingList = pos.filter(po => !(po.status && po.status.includes('Received')));
    ...
    // Search now looks at the Shipment ID as well
    const filteredPOs = activeList.filter(po =>
        po.po_number.toLowerCase().includes(searchTerm.toLowerCase()) ||
        po.shipment_id?.toLowerCase().includes(searchTerm.toLowerCase())
    );
                                    ...
                                    <div key={po.id} onClick={() => !isCompleted && handleSelectPO(po)} className={`group bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border ${isCompleted ? 'border-emerald-500/50' : 'border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl cursor-pointer'} transition-all relative overflow-hidden`}>
                                        ...
                                        {isCompleted && (
                                            <div className="absolute inset-0 bg-emerald-50/10 dark:bg-slate-900/50 backdrop-blur-[1px] flex flex-col items-center justify-center z-10 p-4">
                                                ...
                                                <p className="text-xs text-slate-500 font-bold mt-3 uppercase tracking-wider bg-white dark:bg-slate-800 px-3 py-1 rounded-full">{po.shipment_id}</p>
                                            </div>
                                        )}
                                        ...
                                        {/* 📦 NEW: PROMINENT SHIPMENT ID */}
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-black text-2xl text-blue-600 dark:text-blue-400 mb-1 leading-tight tracking-widest">{po.shipment_id}</h3>
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">REF PO: {po.po_number}</p>

                                        <div className="flex justify-between items-center text-[10px] md:text-xs font-bold border-t border-slate-100 dark:border-slate-800 pt-3">
                                            ...
                                            <span className="text-slate-400 truncate max-w-[120px]">{po.supplier_email}</span>
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden relative">
                        ...
                        <div className="lg:w-1/3 bg-slate-50 dark:bg-slate-800/30 p-5 md:p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 flex flex-col">
                            ...
                            <div className="flex justify-between items-start mb-1">
                                <div>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Receiving Shipment</p>
                                    <h3 className="text-3xl font-black text-blue-600 dark:text-blue-400 break-all pr-2 tracking-widest">{selectedPO.shipment_id}</h3>
                                    <p className="text-xs text-slate-500 font-medium mt-1 uppercase">REF PO: {selectedPO.po_number}</p>
                                </div>
                            ...
                            <div className="flex gap-2 mb-3">
                                <button onClick={() => setScanning(true)} className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-2 text-sm">
                                    <ScanBarcode className="w-4 h-4" /> Scan Items
                                </button>
