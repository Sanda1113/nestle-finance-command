...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const DocumentCard = ({ title, data, borderColor, themeColor }) => {
        ...
        return (
            <div className={`bg-white dark:bg-slate-900 rounded-xl shadow-sm border-t-[3px] ${borderColor} flex flex-col transition-all hover:shadow-md`}>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 font-sans">
            {/* Header */}
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 p-4 px-6 flex flex-wrap justify-between items-center sticky top-0 z-20 shadow-sm">
                <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center shadow-md">
                        <span className="text-white font-bold text-lg">N</span>
                    </div>
                    <h1 className="text-xl font-extrabold tracking-tight bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Nestle<span className="text-slate-800 dark:text-slate-200">Supplier</span></h1>
                </div>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-full">
                        ...
                        <span className="text-xs font-medium text-slate-600 dark:text-slate-300 hidden sm:block">{user.name || user.email}</span>
                    </div>
                    <button onClick={onLogout} className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 text-slate-600 dark:text-slate-300 rounded-full text-xs font-semibold transition-all flex items-center gap-1">🚪 Logout</button>
                </div>
            </div>
            ...
            <div className="p-4 md:p-6 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs uppercase text-slate-500 font-semibold tracking-wider">Total POs</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{totalPOs}</p>
                            </div>
                            <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-blue-600">📄</div>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs uppercase text-slate-500 font-semibold tracking-wider">Total Value</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{formatCurrency(totalPOValue)}</p>
                            </div>
                            <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-600">💰</div>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs uppercase text-slate-500 font-semibold tracking-wider">Pending</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{pendingPOs}</p>
                            </div>
                            <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center text-amber-600">⏳</div>
                        </div>
                    </div>
                    <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-xs uppercase text-slate-500 font-semibold tracking-wider">Matched Invoices</p>
                                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{totalMatched}</p>
                            </div>
                            <div className="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 rounded-full flex items-center justify-center text-purple-600">✅</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-6">
                        {/* Tabs */}
                        <div className="flex flex-wrap gap-2 bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm p-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
                            {[
                            ...
                            <div className="animate-in fade-in duration-300">
                                <div className="mb-4 flex flex-wrap justify-between items-center gap-3">
                                    ...
                                    <div className="relative">
                                        <input
                                            type="text"
                                            placeholder="Search PO # or amount..."
                                            value={searchTerm}
                                            onChange={(e) => setSearchTerm(e.target.value)}
                                            className="pl-9 pr-3 py-1.5 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 focus:ring-2 focus:ring-purple-500"
                                        />
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {filteredPOs.length === 0 ?
                                        <p className="col-span-full text-sm text-slate-500 p-8 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-center">No orders found.</p>
                                        : filteredPOs.map(po => (
                                            <div key={po.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 flex flex-col hover:shadow-lg transition-all group">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div>
                                                        <p className="text-sm font-bold text-slate-800 dark:text-slate-200">{po.po_number}</p>
                                                        <p className="text-xs text-slate-400 mt-0.5">{new Date(po.created_at).toLocaleDateString()}</p>
                                                    </div>
                                                    {po.is_downloaded ?
                                                        <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 ring-1 ring-emerald-500/20 rounded-full text-[10px] font-bold">Downloaded</span>
                                                        : <span className="px-2 py-0.5 bg-blue-50 text-blue-600 ring-1 ring-blue-500/20 animate-pulse rounded-full text-[10px] font-bold">New</span>
                                                    }
                                                </div>
                                                <p className="text-lg font-bold text-slate-800 dark:text-slate-200 mt-1">{formatCurrency(po.total_amount, po.po_data?.currency)}</p>
                        ...
                        {mode === 'boq' && (
                            <div className="max-w-2xl animate-in fade-in duration-300">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                                ...
                                {matchStatus === 'Submitted' && resultData && (
                                    <div className="mt-6">
                                        <div className="p-3 mb-4 bg-emerald-50 border border-emerald-200 rounded-lg text-sm text-emerald-700 text-center font-medium flex items-center justify-center gap-2">✅ Sent to Procurement Team</div>
                            ...
                            <div className="max-w-3xl animate-in fade-in duration-300">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row gap-4">
                                    <div className="flex-1">
                                        ...
                                        <input type="file" accept=".pdf, image/*, .xlsx, .xls, .csv" onChange={(e) => setInvoiceFile(e.target.files[0])} className="block w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:bg-blue-50 file:text-blue-700 cursor-pointer border border-slate-200 dark:border-slate-700 rounded-md p-1.5" />
                                    </div>
                                    <div className="flex-1">
                                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">📑 PO</label>
                                        <input type="file" accept=".pdf, image/*, .xlsx, .xls, .csv" onChange={(e) => setPoFile(e.target.files[0])} className="block w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-full file:border-0 file:bg-purple-50 file:text-purple-700 cursor-pointer border border-slate-200 dark:border-slate-700 rounded-md p-1.5" />
                                    </div>
                                </div>
                        ...
                        {mode === 'logs' && (
                            <div className="animate-in fade-in duration-300">
                                ...
                                <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-5">
                                    {myLogs.length === 0 ? <p className="text-center text-sm text-slate-500 py-4">No activity yet.</p> : (
                                        <div className="relative border-l-2 border-slate-100 dark:border-slate-800 ml-3 space-y-6 pb-2">
                                            {myLogs.map((log) => (
                                                <div key={log.id} className="relative pl-5">
                                                    <div className={`absolute w-3 h-3 rounded-full -left-[7px] top-1.5 ring-4 ring-white dark:ring-slate-900 ${log.status?.includes('Approve') || log.status?.includes('Generated') || log.status === 'Downloaded' ? 'bg-emerald-500' :
                                                            log.status?.includes('Reject') || log.status?.includes('Discrepancy') ? 'bg-red-500' : 'bg-blue-500'
                                                        }`}></div>
                                                    <div className="flex justify-between items-start">
                                                        ...
                                                        <div className="text-right">
                                                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ring-1 ring-inset ${log.status?.includes('Approve') || log.status?.includes('Generated') || log.status === 'Downloaded' ? 'bg-emerald-50 text-emerald-600 ring-emerald-500/20' :
                                                                    log.status?.includes('Reject') || log.status?.includes('Discrepancy') ? 'bg-red-50 text-red-600 ring-red-500/20' : 'bg-amber-50 text-amber-600 ring-amber-500/20'
                                                                }`}>{log.status}</span>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
                            <h3 className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 text-lg mb-3">⚡ Quick Actions</h3>
                            <div className="space-y-2">
                                <button onClick={() => { setMode('boq'); setBoqFile(null); }} className="w-full py-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-lg text-sm font-medium hover:shadow-md transition-all flex items-center justify-center gap-2">📤 Submit Quote</button>
                                <button onClick={() => { setMode('match'); setInvoiceFile(null); setPoFile(null); }} className="w-full py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-lg text-sm font-medium hover:shadow-md transition-all flex items-center justify-center gap-2">🔗 Match Invoice & PO</button>
                                <button onClick={() => { setMode('logs'); }} className="w-full py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-200 transition-all flex items-center justify-center gap-2">📜 View Timeline</button>
                            </div>
                        </div>

                        {/* Recent Activity */}
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
                            <h3 className="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 text-lg mb-3">🕒 Recent Activity</h3>
                            {recentLogs.length === 0 ? (
                                <p className="text-sm text-slate-500 text-center py-3">No recent activity</p>
                            ) : (
                                <div className="space-y-3">
                                    {recentLogs.map(log => (
                                        <div key={log.id} className="flex items-start gap-2 text-sm border-b border-slate-100 dark:border-slate-800 pb-2 last:border-0">
                                            <div className="w-6 text-center">
                                                {log.status?.includes('Approve') ? '✅' : log.status?.includes('Reject') ? '❌' : '⏳'}
                                            </div>
                                            <div className="flex-1">
                                                <p className="font-medium text-slate-700 dark:text-slate-200 text-xs">{log.action}</p>
                        ...
                        {/* Pending POs */}
                        {pendingPOs > 0 && (
                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 rounded-xl border border-amber-200 dark:border-amber-800 p-5 shadow-sm">
                                <h3 className="font-bold text-amber-800 dark:text-amber-200 flex items-center gap-2 text-lg mb-2">⚠️ Attention Needed</h3>
                                <p className="text-amber-700 dark:text-amber-300 text-sm">You have {pendingPOs} unread PO(s). Download them to proceed.</p>
                                <button onClick={() => setMode('inbox')} className="mt-3 text-xs font-bold text-amber-700 dark:text-amber-300 underline">View POs →</button>
                            </div>
                        )}

                        {/* Tips */}
                        <div className="bg-slate-100 dark:bg-slate-800 rounded-xl p-4 text-center">
                            <p className="text-xs text-slate-500 dark:text-slate-400">💡 Pro tip: Upload BOQ to get official PO within 24h.</p>
                        </div>
                    </div>
                </div>

                {error && (
                    <div className="fixed bottom-5 right-5 bg-red-100 border border-red-300 text-red-700 px-4 py-2 rounded-lg shadow-lg text-sm z-50">
                        ⚠️ {error}
                        <button onClick={() => setError(null)} className="ml-3 text-red-500">✖</button>
                    </div>
                )}
                {dbStatus && (
                    <div className="fixed bottom-5 left-5 bg-emerald-100 border border-emerald-300 text-emerald-700 px-4 py-2 rounded-lg shadow-lg text-sm z-50">
                        ✅ {dbStatus}
                    </div>
                )}
            </div>
