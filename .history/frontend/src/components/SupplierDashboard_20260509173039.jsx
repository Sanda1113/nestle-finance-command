...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const steps = useMemo(() => [
        ...
        },
        {
            ...
            body: 'Access the Payment Calender to view your payment calendar or request early funds.',
    ...
    // Tier logic – based on worst category of rejections
    ...
    // Tier display properties
    const tierLabel = trustTier === 1 ? 'Strategic Partner' : trustTier === 2 ? 'Standard Tier' : 'High Risk Partner';
    const tierColor = trustTier === 1 ? 'text-emerald-400' : trustTier === 2 ? 'text-blue-400' : 'text-red-400';
    const tierBg = trustTier === 1 ? 'bg-emerald-900/30' : trustTier === 2 ? 'bg-blue-900/30' : 'bg-red-900/30';
    const tierIcon = trustTier === 1 ? ShieldCheck : trustTier === 2 ? Zap : ShieldAlert;
    ...
    const handleAcceptEarlyPayout = async (payoutId, payoutAmount) => {
        ...
        try {
            ...
            setDialog({ title: 'Success', message: 'Request accepted! Funds are being processed.', type: 'info' });
            fetchData();
        } catch (err) {
            alert('Failed to process payout process.');
        } finally {
            setLoading(false);
        }
    };
        ...
        try {
            ...
            if (res.data.success) {
                ...
                if (isSandboxMode) {
                    ...
                    const docNumber = data.invoiceNumber !== 'Not Found' ? data.invoiceNumber : `BOQ-${Date.now().toString().slice(-6)}`;
                    setDbStatus(`✅ BOQ submitted. Reference: ${docNumber}. Waiting for Finance approval.`);
                    setResultData(prev => ({ ...prev, invoiceNumber: docNumber }));
                }
            }
    ...
    const handlePrintRemittance = (event, tx) => {
        const printWindow = window.open('', '', 'width=800,height=900');
        const html = `
        <html>
        <head>
            <title>Remittance Advice - ${event.bankRef}</title>
            <style>
                body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #1e293b; padding: 30px; font-size: 13px; }
                .header { text-align: center; margin-bottom: 30px; }
                .header h1 { color: #0f172a; margin: 0; font-size: 24px; }
                .header p { color: #64748b; margin: 5px 0 0; text-transform: uppercase; font-size: 11px; font-weight: bold; }
                .content { border-top: 2px solid #cbd5e1; padding-top: 20px; }
        ...
        <body>
            <div class="header">
                <h1>Nestlé Finance</h1>
                <p>Official Remittance Advice</p>
            </div>
            <div class="content">
                <div class="row"><span class="label">Date:</span><span class="val">${new Date(event.date).toLocaleDateString()}</span></div>
                <div class="row"><span class="label">Bank Transaction ID:</span><span class="val">${event.bankRef}</span></div>
                <div class="row"><span class="label">Reference PO:</span><span class="val">${tx.poNumber}</span></div>
                <div class="row"><span class="label">Beneficiary:</span><span class="val">${user.email}</span></div>
                <div class="row"><span class="label">Total Amount Paid:</span><span class="val">${formatCurrency(tx.totalAmount, tx.currency)}</span></div>
            </div>
            <div class="footer">
                <p>This is a computer-generated document. No signature is required.</p>
                <p>Transaction processed via Nestlé Finance Command Platform.</p>
            </div>
            ...
            <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
                <div className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 py-3 sm:px-6 sm:py-4 flex justify-between items-center sticky top-0 z-20 shadow-sm">
                    ...
                    <div className="flex items-center gap-1 sm:gap-3">
                        <div className="hidden lg:flex items-center gap-2 bg-slate-800 px-3 py-1.5 rounded-full border border-slate-700">
                            <div className={`flex items-center gap-1.5`} title={tierLabel}>
                                {React.createElement(tierIcon, { className: `w-3.5 h-3.5 ${tierColor}` })}
                                <span className={`text-[10px] font-black uppercase tracking-widest ${tierColor}`}>{tierLabel}</span>
                            </div>
                        </div>
                ...
                <div className="p-4 md:p-6 max-w-7xl mx-auto">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                        ...
                        <div id="tut-stats-trust" className={`bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-800 hover:shadow-md transition-all ${spotlightClass('tut-stats-trust')}`} title="Your supplier reliability rating. High accuracy in BOQs and Invoices keeps this score high.">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-xs uppercase text-slate-400 font-semibold tracking-wider">Vendor Trust Tier</p>
                                    <p className={`text-xl font-black mt-1 ${tierColor}`}>{tierLabel}</p>
                                    <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">Accuracy: {trustScore}%</p>
                                </div>
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg ${tierBg} ${tierColor}`}>
                                    {React.createElement(tierIcon, { className: "w-6 h-6" })}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="flex flex-wrap gap-2 bg-slate-900/60 backdrop-blur-sm p-1.5 rounded-xl border border-slate-800">
                                {[
                                    ...
                                    { id: 'payouts', label: '💸 Payout calender', color: 'indigo', title: 'Access early payment options and view your scheduled payouts' }
                                ].map(tab => {
                                    const colorMap = {
                                                        ...
                                                        <div className="p-4">
                                                            ...
                                                            <div className="mt-3 space-y-1.5 text-xs border-t border-slate-800 pt-3">
                                                                ...
                                                                <div className="flex justify-between items-center">
                                                                    ...
                                                                    <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${String(po.status || '').includes('Paid') || String(po.status || '').includes('Cleared') ? 'bg-emerald-900/50 text-emerald-400 ring-1 ring-emerald-600/30' :
                            ...
                            {mode === 'match' && (
                                <div className="max-w-3xl animate-in fade-in duration-300">
                                    ...
                                    {matchStatus !== 'Pending' && matchStatus !== 'Submitted' && (
                                        <div className="mt-6">
                                            <div className={`p-4 mb-4 rounded-xl text-sm font-bold border transition-all ${matchStatus === 'Matched - Pending Finance Review'
                                                        ...
                                                        {tx.events.some(e => e.isPaid) && (
                                                            <div className="mt-4 flex justify-center">
                                                                <button
                                                                    onClick={(e) => {
                                                                        e.stopPropagation();
                                                                        const paidEvent = tx.events.find(ev => ev.isPaid);
                                                                        handlePrintRemittance(paidEvent, tx);
                                                                    }}
                                                                    className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black rounded-xl transition-all shadow-lg shadow-emerald-500/20 group"
                                                                >
                                                                                ...
                                                                                <div className="flex items-center gap-2">
                                                                                    ...
                                                                                    {event.isPaid && event.bankRef && (
                                                                                        <button
                                                                                            onClick={(e) => {
                                                                                                e.stopPropagation();
                                                                                                handlePrintRemittance(event, tx);
                                                                                            }}
                            ...
                            <div className="flex gap-1 px-4 pt-3 pb-0">
                                {phaseGroups.map((pg, i) => (
                                    <div
                                        key={i}
                                        title={pg.label}
                                        className={`h-1.5 flex-1 rounded-full transition-all duration-300 cursor-pointer ${sandboxTutorialStep >= pg.range[0] && sandboxTutorialStep <= pg.range[1]
                                            ? pg.color + ' shadow-sm'
