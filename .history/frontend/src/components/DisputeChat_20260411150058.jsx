...
import {
    ...
    Download, Tag, Bot, FileSignature, FileSpreadsheet,
...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit, contextData }) {
    ...
    const [aiMessages, setAiMessages] = useState([{
        ...
        message: `Hello! I am the Nestle ${userRole === 'Finance' ? 'Internal Finance' : 'Supplier'} Assistant for ${referenceNumber}. How can I help you today?`,
            ...
            try {
                ...
                if (cleanKey === '') {
                    ...
                    ${userRole === 'Finance'
                         ...
                         - If the status is "Matched - Pending Finance Review", politely tell the agent that the supplier's numbers match perfectly and it is safe for them to manually click "Approve".
                         - If there is a "Discrepancy Detected", advise the agent to ask the supplier for a POD or Credit Note in the Live Chat.`
                            : `- You are a warm, helpful AI Assistant for a Supplier. You are allowed to answer general questions about the supply chain freely.
                         - If the status is "Discrepancy Detected", politely explain that the invoice amounts don't match the PO and ask them to upload a Credit Note in the Live Chat.
                         - If the status is "Matched - Pending Finance Review", reassure them it's successfully matched and just awaiting final manual approval by the finance team.
                         - Invoices are paid Net-30 AFTER warehouse goods are scanned.`
                        }

                    GENERAL INSTRUCTIONS:
                    - Keep responses conversational, highly readable, and concise (2-4 sentences max).
                    - Do not sound like a robot reading a rigid script. Address the user's specific prompt naturally.`;
