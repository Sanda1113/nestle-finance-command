        ...
        <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            <div className="md:w-64 bg-slate-900 dark:bg-slate-900 text-slate-300 flex flex-col shadow-2xl z-10 shrink-0">
                ...
                <div className="p-4 border-t border-slate-800 space-y-2">
                    <button onClick={triggerSync} className="w-full py-2 mb-2 bg-slate-800 hover:bg-blue-600 rounded-lg text-sm font-bold text-white transition-colors flex items-center justify-center gap-2">
...
function ProcurementPortal({ user }) {
    ...
    useEffect(() => {
        fetchBoqs(true);
        const interval = setInterval(() => fetchBoqs(false), 500);
...
function FinancePortal({ user }) {
    ...
    const enrichedRecords = records.map(r => {
        ...
        // 🛡️ DYNAMIC OVERRIDE LOGIC
        if (displayStatus === 'Finance Review Pending' || displayStatus === 'Pending' || !displayStatus) {
            displayStatus = 'Needs Finance Review';
        } else if (displayStatus.includes('Approve')) {
    ...
    const filteredRecords = enrichedRecords.filter(r => {
        ...
        if (filter === 'Pending') return r.displayStatus === 'Pending Warehouse' || r.match_status.includes('Reject') || r.match_status.includes('Discrepancy') || r.displayStatus === 'Needs Finance Review';
                    ...
                    <table className="w-full text-left text-sm min-w-[1100px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredRecords.map((r) => {
                                const isActioned = actionedRecords[r.id] || r.timeline_status === 'Approved - Awaiting Payout' || r.timeline_status === 'Rejected by Finance' || r.displayStatus.includes('Approved');
                                ...
                                return (
                                    <>
                                        <tr key={r.id} className={`hover:bg-slate-50 dark:bg-slate-800/30 transition-colors ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/20' : ''}`}>
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ring-1 ring-inset ${r.displayStatus.includes('Approved') ? 'bg-emerald-50 text-emerald-600 ring-emerald-500/20' : r.displayStatus.includes('Needs Finance Review') || r.displayStatus.includes('Pending') ? 'bg-amber-50 text-amber-600 ring-amber-500/20' : 'bg-red-50 text-red-600 ring-red-500/20'}`}>
                                                    {r.displayStatus}
                                                </span>
                                            </td>
