...
const getStatusCfg = (status) => {
    switch (status) {
        ...
        case 'Early Payment Requested (Pending Review)': return { bg: 'from-purple-400/90 to-pink-600/90', icon: '⚡', glow: 'shadow-[0_0_20px_rgba(192,132,252,0.3)]', label: 'Early Payout (Pending)' };
...
export default function DigitalCalendar({ userRole, userEmail, refreshTrigger, trustTier = 2, onAcceptEarlyPayout, selectedEventId, onSelectEvent }) {
    ...
    const handleAcceptEarlyPayout = async () => {
        ...
        try {
            ...
            await fetchEvents();
            setSelectedEvent(null);
            alert('⚡ Request successfully accepted! Your funds are being transferred.');
        } catch (error) {
    ...
    const confirmReject = async () => {
        ...
        try {
            await axios.post(
                `https://nestle-finance-command-production.up.railway.app/api/sprint2/payouts/${selectedEvent.id}/cancel-early`,
                { reason: rejectReason }
            );
            fetchEvents();
            setSelectedEvent(null);
        ...
        <div className="flex flex-col lg:flex-row gap-6 p-1">
            ...
            <div className="w-full lg:w-80 shrink-0 space-y-6">
                ...
                <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-5 shadow-2xl">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {['Scheduled', 'Paid', 'Hold', 'Early Payment Requested', 'Renegotiated', 'Early Payment Requested (Pending Review)'].map((k) => {
                    ...
                    <div className="relative w-full max-w-md h-full bg-slate-900 border border-slate-800 rounded-[3rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col animate-in slide-in-from-right-10 duration-500">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                            ...
                            <div className="pt-4 space-y-4">
                                {/* Standard payment actions: only for Scheduled, Staged, Pending Finance */}
                                {isFinance &&
                                    (selectedEvent.status === 'Scheduled' ||
                                        selectedEvent.status === 'Staged' ||
                                        selectedEvent.status === 'Pending Finance') && (
                                ...
                                {/* Early payout approval (pending review) */}
            ...
            {showRejectModal && (
                <div className="fixed inset-0 z-[160] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-md" onClick={() => setShowRejectModal(false)}></div>
                    <div className="relative bg-slate-900 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] max-w-md w-full overflow-hidden border border-slate-800 animate-in zoom-in-95 duration-300">
                        <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-800/30">
                            <h3 className="text-lg font-black text-white flex items-center gap-3">
                                <div className="p-2 bg-rose-500/20 rounded-xl"><X className="w-5 h-5 text-rose-400" /></div>
                                Reject Early Payout
                            </h3>
                            <button onClick={() => setShowRejectModal(false)} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-400 transition-colors">
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        <div className="p-6 space-y-4">
                            <div className="bg-rose-50 dark:bg-rose-900/20 text-rose-800 dark:text-rose-300 p-4 rounded-xl text-sm border border-rose-100 dark:border-rose-800/50">
                                Rejecting will return this payout to <strong>Scheduled</strong> status, and the supplier will not receive the early payment.
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Reason for Rejection (optional)</label>
                                <textarea
                                    value={rejectReason}
                                    onChange={(e) => setRejectReason(e.target.value)}
                                    placeholder="Provide a reason…"
                                    rows="3"
                                    className="w-full p-3 bg-slate-800 border border-slate-700 rounded-xl text-sm text-white outline-none focus:border-rose-500 resize-none"
                                />
                            </div>
                            <div className="mt-6 flex gap-3">
                                ...
                                <button
                                    ...
                                    disabled={isUpdating}
                                    className="flex-1 py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl transition-colors shadow-lg shadow-rose-600/20"
                                >
