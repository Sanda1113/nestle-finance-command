...
export default function Upload() {
  ...
  const [dbStatus, setDbStatus] = useState('Not Saved'); // 'Saving...', 'Saved'
  ...
  // 🚀 AUTOMATICALLY CHECK MATCH & SAVE TO DB
  useEffect(() => {
    const processMatchAndSave = async () => {
      if (invoiceResult && poResult) {
        ...
        // 2. Automatically trigger DB Save
        setDbStatus('Saving...');
        try {
    ...
    return (
      <div className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border-t-4 ${borderColor} overflow-hidden h-full flex flex-col transition-colors`}>
        ...
        <div className="p-6 flex-grow flex flex-col">
                  ...
                  <tbody>
                    {data.lineItems.map((item, idx) => (
                      <tr key={idx} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        ...
                        <td className="p-2 font-medium text-slate-800 dark:text-slate-300 max-w-[120px] truncate">{item.description || '-'}</td>
          ...
          <div className="space-y-3 mb-6 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
            <div>
              <p className="text-xs uppercase text-slate-400 dark:text-slate-500 font-bold">Bank Details</p>
              <p className="text-xs font-medium text-slate-700 dark:text-slate-300 truncate">{data.bankDetails}</p>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
          ...
          <div className="lg:col-span-4 xl:col-span-3 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 transition-colors">
              ...
              <input type="file" onChange={(e) => handleFileChange(e, 'invoice')} className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-blue-50 dark:file:bg-blue-900/30 file:text-blue-700 dark:file:text-blue-400 hover:file:bg-blue-100 dark:hover:file:bg-blue-900/50 cursor-pointer" />
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-xl font-bold text-white transition-all shadow-md ${loading || !invoiceFile || !poFile ? 'bg-slate-300 dark:bg-slate-800 cursor-not-allowed shadow-none text-slate-500' : 'bg-slate-800 dark:bg-blue-600 hover:bg-black dark:hover:bg-blue-500'}`}
            >
              {loading ? "Extracting Data..." : "Extract & Compare Data"}
            </button>

            {error && (
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 text-red-600 dark:text-red-400 p-4 rounded-xl text-sm font-medium">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-8 xl:col-span-9 space-y-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
              ...
              <div className="flex items-center gap-3">
                {matchStatus === 'Pending' && <span className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-black rounded-lg uppercase tracking-wide inline-block">Pending</span>}
                {matchStatus === 'Approved' && <span className="px-4 py-2 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-black rounded-lg uppercase tracking-wide inline-block shadow-sm">✅ Approved Match</span>}
                {matchStatus === 'Rejected' && <span className="px-4 py-2 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 font-black rounded-lg uppercase tracking-wide inline-block shadow-sm">❌ Discrepancy</span>}
                {matchStatus === 'Error' && <span className="px-4 py-2 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 font-black rounded-lg uppercase tracking-wide inline-block">⚠️ System Error</span>}

                {/* Auto-Save Indicator */}
                {(matchStatus === 'Approved' || matchStatus === 'Rejected') && (
                  <span className="text-sm font-bold ml-2">
                    {dbStatus === 'Saving...' && <span className="text-slate-400 animate-pulse">Saving to DB...</span>}
                    {dbStatus === 'Saved' && <span className="text-blue-500">🔒 Saved to Ledger</span>}
                    {dbStatus === 'DB Error' && <span className="text-red-500">⚠️ DB Error</span>}
                  </span>
                )}
              </div>
            </div>
            ...
            {(invoiceResult || poResult) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                <DocumentCard title="Invoice Data" data={invoiceResult} borderColor="border-blue-500" themeColor="text-blue-600 dark:text-blue-400" isApproved={matchStatus === 'Approved'} />
                <DocumentCard title="Purchase Order Data" data={poResult} borderColor="border-purple-500" themeColor="text-purple-600 dark:text-purple-400" isApproved={matchStatus === 'Approved'} />
              </div>
            )}
          </div>
        </div>
