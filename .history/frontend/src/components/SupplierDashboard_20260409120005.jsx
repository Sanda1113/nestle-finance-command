                                                ...
                                                return (
                                                    <div key={po.id} className="bg-slate-900 rounded-xl border border-slate-800 p-4 flex flex-col hover:shadow-lg transition-all group">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div>
                                                                <p className="text-sm font-bold text-slate-100">{getShipmentId(po.po_number)}</p>
                                                        ...
                                                        <p className="text-lg font-black text-slate-100 mt-1">{formatCurrency(po.total_amount, po.po_data?.currency)}</p>

                                                        {/* 🚚 DELIVERY ACTION BUTTONS */}
                                                        <div className="flex gap-2 mt-4">
                                                            <button onClick={() => handlePrintPO(po)} className={`flex-1 py-2.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${po.is_downloaded ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-linear-to-r from-purple-600 to-indigo-600 text-white hover:from-purple-500 hover:to-indigo-500 shadow-sm'}`}>
                                                                📄 PDF
                                                            </button>

                                                            {!isDelivered ? (
                                                                <button onClick={() => handleMarkDelivered(po.po_number)} className="flex-1 py-2.5 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1 bg-amber-600 hover:bg-amber-500 text-white shadow-sm border border-amber-500">
                                                                    🚚 Mark Delivered
                                                                </button>
                                                            ) : (
                                                                <div className="flex-1 py-2.5 text-xs font-bold rounded-lg flex items-center justify-center gap-1 bg-emerald-900/30 text-emerald-400 border border-emerald-800/50">
                                                                    ✅ At Dock
                                                                </div>
                                                            )}
                                                        </div>
                                                                ...
                                                                <div className="flex justify-between items-start">
                                                                    ...
                                                                    <div className="text-right flex flex-col items-end gap-2">
                                                                        ...
                                                                        {hasDispute && (
                                                                            ...
                                                                            >
                                                                                {expandedLog === log.id ? 'Close Chat' : '💬 View Dispute Chat'}
                                                                            </button>
                                                                        )}
