...
function ProcurementPortal() {
    ...
    // Map to track local processing state to prevent 1-second refresh overwrites
    ...
    useEffect(() => {
        fetchBoqs(true);
        const interval = setInterval(() => fetchBoqs(false), 1000); // 1-second silent polling
    ...
    const generatePO = async (id) => {
        ...
        // DB polling will eventually clear processing state automatically
    };
    ...
    // Generate unique list of suppliers by Email
...
function FinancePortal() {
    ...
    useEffect(() => {
        fetchData(true);
        const interval = setInterval(() => fetchData(false), 1000); // 1-second silent polling
        return () => clearInterval(interval);
    }, []);
                    ...
                    <table className="w-full text-left text-sm min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredRecords.map((r) => {
                                const isActioned = r.match_status === 'Approved' || r.match_status === 'Rejected';
                                // Find original BOQ that generated this PO
