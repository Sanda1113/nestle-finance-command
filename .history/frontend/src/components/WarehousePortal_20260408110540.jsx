...
export default function WarehousePortal({ user, onLogout }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300 relative">
            ...
            {/* Added pb-24 padding at the bottom so the mobile sticky footer doesn't cover the last item */}
                ...
                {loading ? (
                    ...
                    <div className="space-y-4 md:space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 max-w-2xl">
                            ...
                            <button onClick={() => setScanning(!scanning)} className={`w-full sm:w-auto px-4 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${scanning ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-slate-800 dark:bg-slate-800 text-white hover:bg-slate-700'}`}>
                                <ScanBarcode className="w-5 h-5 shrink-0" /> {scanning ? 'Stop Scanner' : 'Hardware Scan'}
                            </button>
                        </div>
