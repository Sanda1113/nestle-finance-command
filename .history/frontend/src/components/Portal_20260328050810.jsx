...
function FinancePortal() {
    ...
    // 🚀 NEW: Tracks exactly which buttons have been clicked in this session
    ...
    const handleManualOverride = async (id, decision) => {
        // 🚀 Ensure confirmation works and prevents accidental clicks
        ...
        // Instantly disable the button in the UI
        ...
        try {
            ...
            // If API fails, re-enable the button
    ...
    const filteredRecords = records.filter(r => {
        ...
        if (filter === 'Pending') return r.match_status.includes('Reject'); // Discrepancies act as pending
        if (filter === 'Approved') return r.match_status === 'Approved';
        return false;
    });
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Resolve discrepancies and manually override automated AI decisions.</p>
                </div>
                <div className="flex gap-2">
                    <select value={filter} onChange={(e) => setFilter(e.target.value)} className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="All">All Documents</option>
                        <option value="Pending">Needs Review (Discrepancies)</option>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden overflow-x-auto">
                {loading ? (
                    ...
                    <table className="w-full text-left text-sm min-w-[900px]">
                        {/* 🚀 FIXED: Added the new 'Match Details' Column */}
                        <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 uppercase text-xs font-bold tracking-wider">
                            <tr>
                                ...
                                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-center">Match Details</th>
                                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredRecords.map((r) => {
                                // 🚀 FIXED: Button locks if manually acted upon (UI state OR database timeline state)
                                ...
                                return (
                                    <>
                                        <tr key={r.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/20' : ''}`}>
                                            <td className="p-4 font-mono text-xs text-slate-400">{new Date(r.processed_at).toLocaleDateString()}</td>

                                            {/* Removed the messy inline button from here */}
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ring-1 ring-inset ${r.match_status === 'Approved' ? 'bg-emerald-50 text-emerald-600 ring-emerald-500/20' : 'bg-red-50 text-red-600 ring-red-500/20'}`}>
                                                    {r.match_status}
                                                </span>
                                            </td>

                                            {/* 🚀 NEW: Clean Dedicated "View Match" Button Column */}
                                            <td className="p-4 text-center">
                                                <button
                                                    onClick={() => setExpandedRow(isExpanded ? null : r.id)}
                                                    className={`px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded transition-colors ${isExpanded ? 'bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300' : 'bg-blue-50 hover:bg-blue-100 text-blue-600 dark:bg-blue-500/10 dark:hover:bg-blue-500/20 dark:text-blue-400'}`}
                                                >
                                        ...
                                        {/* 🚀 EXTANDED ROW: 3-Way Match Document Viewer (8 Columns wide now) */}
                                        {isExpanded && (
                                            <tr className="bg-slate-100/50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                                                <td colSpan="8" className="p-4 px-6">
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        <div className="bg-gradient-to-br from-blue-50 to-white dark:from-slate-950 dark:to-slate-900 p-3.5 rounded-xl border border-blue-200 dark:border-blue-900/50 shadow-sm relative overflow-hidden">
                                                            ...
                                                            <div className="flex justify-between items-center mb-1">
                                                                ...
                                                                {relatedBoq ? (
                                                                    <button onClick={() => handlePrintDocument('Original Quote / BOQ', { ...relatedBoq, lineItems: relatedBoq.line_items, invoiceNumber: relatedBoq.document_number, totalAmount: relatedBoq.total_amount })} className="text-[10px] font-bold text-blue-600 hover:underline">📄 Print</button>
                                                                ) : (
                                                        ...
                                                        <div className="bg-gradient-to-br from-purple-50 to-white dark:from-slate-950 dark:to-slate-900 p-3.5 rounded-xl border border-purple-200 dark:border-purple-900/50 shadow-sm relative overflow-hidden">
                                                            ...
                                                            <div className="flex justify-between items-center mb-1">
                                                                ...
                                                                <button onClick={() => handlePrintDocument('Purchase Order', r.po_data)} className="text-[10px] font-bold text-purple-600 hover:underline">📄 Print</button>
                                                            </div>
                                                            <p className="text-xl font-black text-slate-800 dark:text-slate-100">{formatCurrency(r.po_total)}</p>
                                                        </div>

                                                        <div className="bg-gradient-to-br from-emerald-50 to-white dark:from-slate-950 dark:to-slate-900 p-3.5 rounded-xl border border-emerald-200 dark:border-emerald-900/50 shadow-sm relative overflow-hidden">
                                                            ...
                                                            <div className="flex justify-between items-center mb-1">
                                                                ...
                                                                <button onClick={() => handlePrintDocument('Invoice Data Extract', r.invoice_data)} className="text-[10px] font-bold text-emerald-600 hover:underline">📄 Print</button>
                                                            </div>
