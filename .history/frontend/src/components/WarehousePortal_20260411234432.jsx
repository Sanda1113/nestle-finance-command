            ...
            try {
                const scanner = new Html5QrcodeScanner(
                    "reader",
                    {
                        ...
                        // Remove supportedScanTypes – defaults to both camera and file
                    },
                    false
                );
                ...
                scanner.render(
                    ...
                    (error) => {
                        ...
                        if (process.env.NODE_ENV === 'development' && !error?.includes?.('No MultiFormat Readers')) {
...
export default function WarehousePortal({ user, onLogout }) {
    ...
    // 🆕 Memoized callbacks for scanner
    const handleScanSuccess = useCallback((text) => {
        setScanning(false);
        processScanResult(text);
    }, []); // processScanResult uses latestState ref, so no dependency needed
