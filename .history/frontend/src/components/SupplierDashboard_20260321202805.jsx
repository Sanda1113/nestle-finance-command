...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (invoiceRes.data.success && poRes.data.success) {
                ...
                // 🛡️ STRICT MATCHING LOGIC: Fixes the false positive "Approved" bug
                // Both documents must have a total > 0, AND they must match exactly.
                let status = 'Rejected';
    ...
    // The card component to display extracted data
                                    ...
                                    <tbody>
                                        {data.lineItems.map((item, idx) => (
                                            <tr key={idx} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                                ...
                                                <td className="p-2 text-right text-slate-600 dark:text-slate-400">${item.unitPrice?.toFixed(2)}</td>
                                                <td className="p-2 text-right font-bold text-slate-800 dark:text-slate-200">${item.amount?.toFixed(2)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
