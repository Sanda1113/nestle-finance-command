            ...
            <div className="grid grid-cols-1 gap-6">
                {loading ? (
                    ...
                    filteredBoqs.map(boq => {
                        ...
                        return (
                            <div key={boq.id} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col md:flex-row transition-all hover:shadow-md">
                                    ...
                                    <div className="overflow-y-auto max-h-48 border border-slate-100 dark:border-slate-700 rounded-lg">
                                        <table className="w-full text-left text-xs">
                                            <thead className="bg-slate-50 dark:bg-slate-800 sticky top-0">
                                                <tr>
                                                    <th className="p-3 font-bold text-slate-500">Qty</th>
                                                    <th className="p-3 font-bold text-slate-500">Item</th>
                                                    <th className="p-3 font-bold text-slate-500 text-right">Unit Price</th>
...
function FinancePortal() {
    ...
    const pendingDiscrepancies = records.filter(r => r.match_status && r.match_status.includes('Reject')).length;
    const approvedRecords = records.filter(r => r.match_status === 'Approved').length;
    const totalValue = records.reduce((sum, r) => sum + (Number(r.invoice_total) || 0), 0);
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold tracking-wider">Total Documents</p>
                            <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{totalRecords}</p>
                        </div>
                        <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400">📊</div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold tracking-wider">Pending Discrepancies</p>
                            <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{pendingDiscrepancies}</p>
                        </div>
                        <div className="w-10 h-10 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center text-amber-600 dark:text-amber-400">⚠️</div>
                    </div>
                </div>
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all">
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-xs uppercase text-slate-500 dark:text-slate-400 font-semibold tracking-wider">Approved</p>
                            <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{approvedRecords}</p>
                        </div>
                        <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400">✅</div>
                    </div>
                </div>
                    ...
                    <table className="w-full text-left text-sm min-w-[900px]">
                        <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 uppercase text-xs font-bold tracking-wider">
                            <tr>
                                ...
                                <th className="p-4 border-b border-slate-200 dark:border-slate-800">Preview</th>
                                <th className="p-4 border-b border-slate-200 dark:border-slate-800 text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {filteredRecords.map((r) => {
                                const isActioned = r.timeline_status === 'Approved - Awaiting Payout' || r.timeline_status === 'Rejected by Finance';
                                const relatedBoq = boqs.find(b => b.status.includes(r.po_number));

                                return (
                                    <>
                                        <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 cursor-pointer" onClick={() => setExpandedRow(expandedRow === r.id ? null : r.id)}>
                                            ...
                                            <td className="p-4">
                                                <span className={`px-3 py-1 rounded-full text-xs font-black uppercase ${r.match_status === 'Approved' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-300' : 'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300'}`}>
                                                    {r.match_status}
                                                </span>
                                            </td>
                                            // Inside FinancePortal component, in the table body rendering:
                                            ...
                                            <td className="p-4">
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            handlePrintDocument('Invoice Data Extract', r.invoice_data);
                                                        }}
                                                        className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-800/50 text-blue-700 dark:text-blue-300 text-xs font-medium rounded-md transition-colors whitespace-nowrap"
                                                    >
                                                        📄 Download Invoice
                                                    </button>
                                                    {/* PO Preview Button */}
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            handlePrintDocument('Purchase Order', r.po_data);
                                                        }}
                                                        className="px-2 py-1 bg-purple-100 dark:bg-purple-900/30 hover:bg-purple-200 dark:hover:bg-purple-800/50 text-purple-700 dark:text-purple-300 text-xs font-medium rounded-md transition-colors whitespace-nowrap"
                                                    >
                                                        📑 Download PO
                                                    </button>
                                                    {/* Optional: BOQ Preview if available */}
                                                    {relatedBoq && (
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                handlePrintDocument('Original Quote / BOQ', { ...relatedBoq, lineItems: relatedBoq.line_items, invoiceNumber: relatedBoq.document_number, totalAmount: relatedBoq.total_amount });
                                                            }}
                                                            className="px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 hover:bg-emerald-200 dark:hover:bg-emerald-800/50 text-emerald-700 dark:text-emerald-300 text-xs font-medium rounded-md transition-colors whitespace-nowrap"
                                                        >
                                                            📜 Download BOQ
                                                        </button>
                                                    )}
                                            ...
                                            <td className="p-4 text-right space-x-2">
                                                <button onClick={(e) => { e.stopPropagation(); handleManualOverride(r.id, 'Approved'); }} disabled={isActioned} className={`px-3 py-1 font-bold rounded text-xs transition-colors ${isActioned ? 'bg-slate-200 text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600' : 'bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500 hover:text-white text-slate-600 dark:text-slate-300'}`}>Approve</button>
                                                <button onClick={(e) => { e.stopPropagation(); handleManualOverride(r.id, 'Rejected'); }} disabled={isActioned} className={`px-3 py-1 font-bold rounded text-xs transition-colors ${isActioned ? 'bg-slate-200 text-slate-400 cursor-not-allowed dark:bg-slate-800 dark:text-slate-600' : 'bg-slate-100 dark:bg-slate-800 hover:bg-red-500 hover:text-white text-slate-600 dark:text-slate-300'}`}>Reject</button>
                                            </td>
                                        </tr>
                                        {/* Expanded Row */}
                                        {expandedRow === r.id && (
                                            <tr className="bg-slate-50 dark:bg-slate-800/20">
                                                <td colSpan="8" className="p-6 border-b border-slate-200 dark:border-slate-800">
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                        ...
                                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-blue-200 dark:border-blue-900/50 shadow-sm">
                                                            ...
                                                            <p className="text-sm"><strong>Quote Total:</strong> {relatedBoq ? formatCurrency(relatedBoq.total_amount) : 'N/A'}</p>
                                                        </div>
                                                        ...
                                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-purple-200 dark:border-purple-900/50 shadow-sm">
                                                            ...
                                                            <p className="text-sm"><strong>PO Total:</strong> {formatCurrency(r.po_total)}</p>
                                                        </div>
                                                        ...
                                                        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-emerald-200 dark:border-emerald-900/50 shadow-sm">
                                                            ...
                                                            <p className="text-sm"><strong>Inv Total:</strong> {formatCurrency(r.invoice_total)}</p>
                                                            <p className="text-sm mt-2"><strong>Variance vs PO:</strong> <span className={r.invoice_total === r.po_total ? 'text-emerald-500 font-bold' : 'text-red-500 font-bold'}>{formatCurrency(Math.abs(r.invoice_total - r.po_total))}</span></p>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
