...
function ProcurementPortal({ user }) {
    ...
    useEffect(() => {
        fetchBoqs(true);
        const interval = setInterval(() => fetchBoqs(false), 500); // ⚡ 0.5s REFRESH
...
function FinancePortal({ user }) {
    ...
    useEffect(() => {
        fetchData(true);
        const interval = setInterval(() => fetchData(false), 500); // ⚡ 0.5s REFRESH
        return () => clearInterval(interval);
    }, []);
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex flex-wrap justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        className="px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide rounded bg-slate-100 dark:bg-slate-800 hover:bg-emerald-50 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-slate-600 dark:text-slate-300"
                                                    >
                                                        Approve
                                                    </button>
                                                    <button
