...
export default function WarehousePortal({ user, onLogout }) {
    ...
    const processScanResult = (decodedText) => {
        ...
        if (!state.selectedPO) {
            // Dashboard mode: Look for PO (Exclude 'Completed' POs)
            const matchedPO = state.pos.find(p => p.po_number.toLowerCase() === decodedText.toLowerCase());

            if (matchedPO && matchedPO.status !== 'Completed') {
                setSearchTerm(decodedText);
                handleSelectPO(matchedPO);
            } else if (matchedPO && matchedPO.status === 'Completed') {
                alert(`⚠️ PO ${decodedText} has already been marked as Completed and Locked.`);
            } else {
                setDetectedProduct({
                    barcode: decodedText,
    ...
    const handleImageUpload = async (event) => {
        ...
        try {
            ...
            setTimeout(() => {
                setIsProcessingImage(false);
                const mockBarcode = `893452${Math.floor(Math.random() * 1000)}`;
    ...
    const submitGRN = async () => {
        ...
        const payload = {
            ...
            receivedBy: user?.email || 'Warehouse User',
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300 relative">
            ...
            {scanning && (
                <BarcodeScannerUI
                    onScanSuccess={(text) => { setScanning(false); processScanResult(text); }}
            ...
            {detectedProduct && (
                <div className="fixed inset-0 z-[150] bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl border border-slate-200 dark:border-slate-800 relative">
                        <button onClick={() => setDetectedProduct(null)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X className="w-6 h-6" /></button>
                        <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center mx-auto mb-4"><Package className="w-8 h-8" /></div>
                        ...
                        <button onClick={() => setDetectedProduct(null)} className="w-full mt-6 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-colors">Dismiss</button>
                    </div>
                </div>
            )}
            ...
            <div className="bg-slate-900/90 backdrop-blur-md border-b border-slate-800 p-3 md:p-4 px-4 md:px-6 flex justify-between items-center sticky top-0 z-30 shadow-sm shrink-0">
                <div className="flex items-center space-x-2 md:space-x-3">
                    <div className="w-8 h-8 md:w-10 md:h-10 bg-white rounded-xl flex items-center justify-center shadow-md p-1.5 border border-slate-700 shrink-0"><img src="/nestle-logo.svg" alt="Nestle" className="w-full h-full object-contain" /></div>
                    <div>
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    {isOffline ? <span className="flex items-center gap-1 bg-red-500/20 text-red-400 border border-red-500/30 px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold animate-pulse"><WifiOff className="w-3 h-3 shrink-0" /> <span className="hidden sm:inline">Offline Mode</span></span> : <span className="hidden sm:flex items-center gap-1 bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold"><CheckCircle2 className="w-3 h-3 shrink-0" /> Online</span>}
                    <div className="w-px h-5 md:h-6 bg-slate-700 mx-1 md:mx-2"></div>
                    <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-1.5 md:p-2 text-slate-300 hover:bg-slate-800 rounded-lg transition-colors">{isDarkMode ? <Sun className="w-4 h-4 md:w-5 md:h-5" /> : <Moon className="w-4 h-4 md:w-5 md:h-5" />}</button>
                    {onLogout && <button onClick={onLogout} className="flex items-center gap-1.5 px-2 md:px-3 py-1.5 md:py-2 bg-red-900/40 hover:bg-red-600 text-white rounded-lg text-xs md:text-sm font-bold transition-colors"><LogOut className="w-4 h-4 shrink-0" /> <span className="hidden sm:block">Exit</span></button>}
                </div>
            </div>
            ...
            <div className="flex-1 p-4 md:p-8 max-w-7xl mx-auto w-full pb-28 md:pb-8">
                ...
                {isSyncing && <div className="mb-4 md:mb-6 bg-blue-500 text-white p-3 rounded-xl flex items-center justify-center gap-2 font-bold shadow-lg text-sm"><RefreshCw className="w-4 h-4 animate-spin shrink-0" /> Syncing offline data...</div>}
                {syncQueue.length > 0 && !isOffline && !isSyncing && <button onClick={syncPendingGRNs} className="mb-4 md:mb-6 w-full bg-amber-500 hover:bg-amber-600 text-white p-3 rounded-xl flex items-center justify-center gap-2 font-bold shadow-lg transition-colors text-sm"><RefreshCw className="w-4 h-4 shrink-0" /> {syncQueue.length} pending GRNs. Tap to sync.</button>}

                <div className="flex flex-col md:flex-row justify-between md:items-end gap-2 md:gap-4 mb-4 md:mb-6">
                    <div>
                        <h2 className="text-2xl md:text-3xl font-black text-slate-800 dark:text-white flex items-center gap-2 md:gap-3"><Truck className="text-blue-500 w-6 h-6 md:w-8 md:h-8 shrink-0" /> Dock Receiving</h2>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4 md:space-y-6">
                        <div className="flex flex-col sm:flex-row gap-3 max-w-2xl">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => setScanning(true)} className="flex-1 sm:flex-none px-4 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all bg-slate-800 text-white hover:bg-slate-700 shadow-md"><ScanBarcode className="w-5 h-5 shrink-0" /> Live Scan</button>
                                <button onClick={() => fileInputRef.current.click()} className="flex-1 sm:flex-none px-4 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all bg-blue-600 text-white hover:bg-blue-700 shadow-md"><Camera className="w-5 h-5 shrink-0" /> Snap Photo</button>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
                            {filteredPOs.map(po => {
                                // 🚀 NEW: Add Completed state UI logic
                                const isCompleted = po.status === 'Completed';
                                return (
                                    <div key={po.id} onClick={() => !isCompleted && handleSelectPO(po)} className={`group bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 transition-all relative overflow-hidden ${isCompleted ? 'opacity-60 cursor-not-allowed' : 'cursor-pointer hover:shadow-xl'}`}>
                                        ...
                                        <div className="flex justify-between items-center text-[10px] md:text-xs font-bold">
                                            ...
                                            {/* 🚀 NEW: Completed Badge */}
                                            <span className={`px-2.5 py-1 rounded-lg uppercase ${isCompleted ? 'bg-emerald-500 text-white' : po.status.includes('Partial') ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                                                {isCompleted ? 'Completed' : po.status.includes('Partial') ? 'Partial' : 'Pending'}
                                            </span>
                                        </div>
                                    </div>
                                )
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden relative">
                        <div className="lg:w-1/3 bg-slate-50 dark:bg-slate-800/30 p-5 md:p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 flex flex-col">
                            <button onClick={() => setSelectedPO(null)} className="flex items-center gap-1.5 text-xs md:text-sm text-slate-500 hover:text-blue-600 font-bold mb-4 md:mb-6 w-fit"><ArrowLeft className="w-4 h-4" /> Back</button>
                            <div className="flex justify-between items-start mb-1">
                                ...
                                <button onClick={() => setBlindMode(!blindMode)} className={`p-2 rounded-lg shrink-0 ${blindMode ? 'bg-blue-100 text-blue-600 ring-2 ring-blue-500/50' : 'bg-slate-200 text-slate-600'}`} title="Toggle Strict Blind Receiving">{blindMode ? <EyeOff className="w-4 h-4 md:w-5 md:h-5" /> : <Eye className="w-4 h-4 md:w-5 md:h-5" />}</button>
                            </div>
                            <div className="flex flex-wrap items-center gap-2 mb-6 md:mb-8"><p className="text-xs md:text-sm text-slate-500 font-medium truncate">{selectedPO.supplier_email}</p></div>
                            ...
                            {!blindMode && (
                                <div className="mb-4 md:mb-8 bg-white dark:bg-slate-900 p-4 md:p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all">
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className="text-base md:text-lg font-black text-blue-500">{receivedItems.reduce((s, i) => s + parseFloat(i.actualQtyReceived), 0)} <span className="text-xs md:text-sm text-slate-400">/ {receivedItems.reduce((s, i) => s + parseFloat(i.qty), 0)}</span></span>
                                    </div>
                                    <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-2 md:h-3 overflow-hidden">
                                        <div className={`h-full rounded-full transition-all duration-500 bg-blue-500`} style={{ width: `${Math.min((receivedItems.reduce((s, i) => s + parseFloat(i.actualQtyReceived), 0) / receivedItems.reduce((s, i) => s + parseFloat(i.qty), 0)) * 100, 100)}%` }}></div>
                                    </div>
                                </div>
                            )}

                            <div className="flex gap-2 mb-3">
                                <button onClick={() => setScanning(true)} className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-2 text-sm"><ScanBarcode className="w-4 h-4" /> Live Scan</button>
                                <button onClick={() => fileInputRef.current.click()} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-2 text-sm"><Camera className="w-4 h-4" /> Photo</button>
                            </div>
                            <div className="hidden lg:block mt-auto space-y-3 pt-4">
                                <button onClick={submitGRN} className="w-full py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl transition-all shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2"><MapPin className="w-5 h-5" /> Sign & Lock GRN</button>
                            </div>
                        </div>
                                        ...
                                        <div key={idx} className={`p-4 md:p-5 rounded-2xl border-2 transition-all bg-white dark:bg-slate-950 ${isHighRisk ? 'border-red-500 shadow-lg shadow-red-500/20' : isShort ? 'border-red-500/50' : isOver ? 'border-amber-500/50' : isPending ? 'border-slate-200 dark:border-slate-800' : 'border-emerald-500/30'}`}>
                                            <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                                                ...
                                                <div className="flex items-center justify-between sm:justify-start gap-1 bg-slate-100 dark:bg-slate-900 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700 w-full sm:w-auto">
                                                    <button onClick={() => handleQtyChange(idx, -1)} className="w-12 h-12 sm:w-10 sm:h-10 flex items-center justify-center bg-white dark:bg-slate-800 rounded-lg text-xl font-black text-slate-600 dark:text-slate-300 shadow-sm shrink-0">-</button>
                        ...
                        <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 p-4 z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                            <button onClick={submitGRN} className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl transition-all shadow-lg flex items-center justify-center gap-2">
