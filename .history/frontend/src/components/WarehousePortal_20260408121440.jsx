...
import { Package, Truck, CheckCircle2, AlertCircle, ScanBarcode, ArrowLeft, Camera, Search, EyeOff, Eye, CalendarDays, Hash, ShieldAlert, ShieldCheck, WifiOff, MapPin, RefreshCw, LogOut, Moon, Sun, X, Image as ImageIcon } from 'lucide-react';
...
export default function WarehousePortal({ user, onLogout }) {
    ...
    const scannerRef = useRef(null); // Holds the active scanner instance
    const fileInputRef = useRef(null); // Ref for the native camera fallback
    ...
    // ==========================================
    // 📸 BARCODE SCANNER LOGIC (LIVE & FILE)
    // ==========================================

    const processScanResult = (decodedText) => {
        if (!selectedPO) {
            // We are on the Dashboard. Look for a PO matching the barcode.
            const matchedPO = pos.find(p => p.po_number.toLowerCase() === decodedText.toLowerCase());
            if (matchedPO) {
            ...
            // We are inside a PO. Add +1 to the item.
            // (In a real app, you would match decodedText to the item's SKU. For MVP, we increment the first item).
            handleQtyChange(0, 1);
            alert(`✅ Scan Successful!\nAdded 1 unit based on barcode: ${decodedText}`);
        }
    };

    // Method 1: Live Video Feed
    useEffect(() => {
        if (scanning) {
            ...
            const timer = setTimeout(() => {
                ...
                html5QrCode.start(
                    ...
                    (decodedText) => {
                        // Success! Stop camera and process.
                    ...
                    alert("Live camera blocked by browser permissions. Please use the 'Snap Photo' button instead.");
                });
            }, 100);

            return () => clearTimeout(timer);
        } else {
            // Cleanup camera when closing overlay
            if (scannerRef.current && scannerRef.current.isScanning) {
                scannerRef.current.stop().catch(console.error);
            }
        }
    }, [scanning, selectedPO, pos]);

    // Method 2: Native Camera App Fallback (Take a Picture)
    const handleImageUpload = async (event) => {
        const file = event.target.files[0];
        if (!file) return;

        try {
            setScanning(false); // Close live scanner if open
            const html5QrCode = new Html5Qrcode("hidden-reader"); // Hidden div
            const decodedText = await html5QrCode.scanFile(file, true);
            processScanResult(decodedText);
        } catch (err) {
            console.error(err);
            alert("Could not detect a clear barcode in that image. Please try again.");
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300 relative">
            ...
            {/* Hidden Div required for Native Camera Photo Upload processing */}
            ...
            {scanning && (
                <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex flex-col items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="flex flex-col items-center mb-6">
                        ...
                        <p className="text-slate-400 text-sm mt-1 text-center">Align barcode within the frame</p>
                    </div>
                    ...
                    <p className="text-slate-500 mt-6 text-sm font-bold uppercase tracking-widest">OR</p>

                    {/* NATIVE CAMERA FALLBACK BUTTON */}
                    <button onClick={() => fileInputRef.current.click()} className="mt-4 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold flex items-center gap-2 border border-slate-700">
                        <Camera className="w-5 h-5" /> Snap Photo Instead
                    </button>
                </div>
            )}
