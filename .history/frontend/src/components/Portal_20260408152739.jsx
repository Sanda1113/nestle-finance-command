...
function FinancePortal({ user }) {
    ...
    const [boqs, setBoqs] = useState([]);
    const [pos, setPOs] = useState([]); // NEW: Needed to check Warehouse Status
    const [loading, setLoading] = useState(true);
    const [filter, setFilter] = useState('All');
    ...
    // NEW: Map records to include the Warehouse GRN status dependency
    const enrichedRecords = records.map(r => {
        ...
        let displayStatus = r.match_status;
        let isPendingWarehouse = false;

        // Lock it to Pending Warehouse if Dock hasn't signed off yet
        if (!isGrnCompleted) {
            displayStatus = 'Pending Warehouse';
            isPendingWarehouse = true;
        }

        return { ...r, displayStatus, isPendingWarehouse, relatedPO };
    });

    const filteredRecords = enrichedRecords.filter(r => {
        if (filter === 'All') return true;
        if (filter === 'Pending') return r.displayStatus === 'Pending Warehouse' || r.match_status.includes('Reject') || r.match_status.includes('Discrepancy');
        if (filter === 'Approved') return r.displayStatus === 'Approved';
        return false;
    });
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Resolve discrepancies and communicate with suppliers.</p>
                </div>
                <div className="flex gap-2">
                    <select value={filter} onChange={(e) => setFilter(e.target.value)} className="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-600 dark:text-slate-300 outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="All">All Documents</option>
                        <option value="Pending">Needs Review (Discrepancies / Dock)</option>
                                ...
                                return (
                                    <>
                                        <tr key={r.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/20' : ''}`}>
                                            ...
                                            <td className="p-4">
                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ring-1 ring-inset ${r.displayStatus === 'Approved' ? 'bg-emerald-50 text-emerald-600 ring-emerald-500/20' : r.displayStatus === 'Pending Warehouse' ? 'bg-amber-50 text-amber-600 ring-amber-500/20' : 'bg-red-50 text-red-600 ring-red-500/20'}`}>
                                                    {r.displayStatus}
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex flex-wrap justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        disabled={isActioned || r.isPendingWarehouse}
                                            ...
                                            <tr className="bg-slate-100/50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
                                                <td colSpan="10" className="p-4 px-6">
                                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                        <div className="flex flex-col gap-4">
                                                            ...
                                                            <div className="bg-linear-to-br from-purple-50 to-white dark:from-slate-950 dark:to-slate-900 p-4 rounded-xl border border-purple-200 dark:border-purple-900/50 shadow-sm relative overflow-hidden">
                                                                ...
                                                                <div className="flex justify-between items-center mb-2">
                                                                    ...
                                                                    <button onClick={() => handlePrintDocument('Purchase Order', r.po_data)} className="px-3 py-1.5 text-xs font-bold text-white bg-purple-500 hover:bg-purple-600 rounded-md shadow-sm transition-colors">📄 Print PDF</button>
                                                                </div>
                                                            ...
                                                            <div className="bg-linear-to-br from-emerald-50 to-white dark:from-slate-950 dark:to-slate-900 p-4 rounded-xl border border-emerald-200 dark:border-emerald-900/50 shadow-sm relative overflow-hidden">
                                                                ...
                                                                <div className="flex justify-between items-center mb-2">
                                                                    ...
                                                                    <button onClick={() => handlePrintDocument('Invoice Data Extract', r.invoice_data)} className="px-3 py-1.5 text-xs font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-md shadow-sm transition-colors">📄 Print PDF</button>
                                                                </div>
                                                        ...
                                                        {/* Right Side: The Chat */}
                                                        <div>
                                                            {r.isPendingWarehouse ? (
                                                                <div className="h-full flex flex-col items-center justify-center p-6 border-2 border-dashed border-amber-200 dark:border-amber-900/50 rounded-2xl bg-amber-50/50 dark:bg-amber-900/10">
                                                                    <Truck className="w-12 h-12 text-amber-500/50 mb-3 animate-pulse" />
                                                                    <p className="text-center font-bold text-amber-700 dark:text-amber-500 mb-1">Awaiting Dock Confirmation</p>
                                                                    <p className="text-center text-xs text-amber-600/70 dark:text-amber-500/70 max-w-[250px]">Dispute resolution is locked until the warehouse physically signs for this delivery.</p>
                                                                </div>
                                                            ) : (
                                                                <DisputeChat
