...
export default function Portal() {
    const [activeTab, setActiveTab] = useState('supplier'); // 'supplier', 'finance', 'analytics'
    ...
    return (
        <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            {/* ================= SIDEBAR NAVIGATION ================= */}
            <div className="md:w-64 bg-slate-900 text-slate-300 flex flex-col shadow-xl z-10">
                ...
                <div className="flex-grow py-6 flex flex-col gap-2 px-4 overflow-y-auto">
                    ...
                    <button
                        ...
                        className={`text-left px-4 py-3 rounded-xl font-medium transition-all ${activeTab === 'supplier' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'}`}
                    >
                        🏢 Supplier Upload
                    </button>
                    ...
                    <button
                        ...
                        className={`text-left px-4 py-3 rounded-xl font-medium transition-all ${activeTab === 'finance' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'}`}
                    >
                        📋 Review Queue
                    </button>
                    ...
                    <button
                        ...
                        className={`text-left px-4 py-3 rounded-xl font-medium transition-all ${activeTab === 'analytics' ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-800 hover:text-white'}`}
                    >
                        📈 Analytics
                    </button>
                </div>
                ...
                <div className="p-4 border-t border-slate-800">
                    <button
                        ...
                        className="w-full py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-bold text-white transition-colors flex items-center justify-center gap-2"
                    >
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (invoiceRes.data.success && poRes.data.success) {
                setInvoiceResult(invoiceRes.data.extractedData); setPoResult(poRes.data.extractedData);
            }
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="mb-6">
                ...
                <p className="text-slate-500 dark:text-slate-400">Submit invoices and POs for automated clearing.</p>
            </div>

            <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-4 transition-colors">
                ...
                <button onClick={handleUpload} disabled={loading || !invoiceFile || !poFile} className="px-8 py-3 md:py-0 bg-slate-800 dark:bg-blue-600 hover:bg-black dark:hover:bg-blue-500 text-white font-bold rounded-xl disabled:bg-slate-300 dark:disabled:bg-slate-800 mt-4 md:mt-6 transition-colors">
                    {loading ? "Extracting..." : "Process AI Match"}
                </button>
            </div>
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden transition-colors overflow-x-auto">
                {loading ? (
                    <div className="p-12 text-center text-slate-500 font-bold animate-pulse">Loading Database Records...</div>
                ) : (
                    <table className="w-full text-left text-sm min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            ...
                            {records.length === 0 && !loading && (
                                <tr><td colSpan="7" className="p-8 text-center text-slate-400 italic">No records found in database.</td></tr>
                            )}
                        </tbody>
                    </table>
                )}
