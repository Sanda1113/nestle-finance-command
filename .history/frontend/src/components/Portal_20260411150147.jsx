...
function FinancePortal({ user }) {
    ...
    const enrichedRecords = records.map(r => {
        ...
        if (displayStatus === 'Matched - Pending Finance Review' || displayStatus === 'Pending' || !displayStatus) {
            displayStatus = 'Matched - Pending Finance Review';
        } else if (displayStatus === 'Discrepancy Detected') {
            displayStatus = 'Discrepancy Detected';
    ...
    const filteredRecords = enrichedRecords.filter(r => {
        ...
        if (filter === 'Pending') return r.displayStatus === 'Matched - Pending Finance Review' || r.displayStatus === 'Discrepancy Detected' || r.displayStatus === 'Pending Warehouse GRN' || r.match_status.includes('Reject') || r.match_status.includes('Discrepancy');
