...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit, contextData }) {
    ...
    const [aiMessages, setAiMessages] = useState([{
        ...
        message: `Hello! I am your tracking assistant for ${referenceNumber}. You can ask me things like "What is the status?" or "When will I get paid?"`,
    ...
    const sendMessage = async (text = newMessage, forceRole = userRole, metadata = null) => {
        ...
        if (chatMode === 'ai') {
            ...
            setTimeout(() => {
                ...
                if (lowerText.includes('status') || lowerText.includes('where is my') || lowerText.includes('update')) {
                    const statusStr = contextData?.status || 'Processing';
                    aiResponse = `According to the system ledger, Shipment/PO ${referenceNumber} is currently marked as: **${statusStr}**.\n\nIf this requires human intervention, please switch to 'Live Agent' mode at the top of the chat.`;
