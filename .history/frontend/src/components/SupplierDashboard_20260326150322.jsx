...
export default function SupplierDashboard({ user, onLogout }) {
    const [mode, setMode] = useState('inbox'); // 'inbox', 'boq', 'match', 'logs'
        ...
        try {
            ...
            if (res.data.success) {
                ...
                await axios.post('https://nestle-finance-command-production.up.railway.app/api/save-boq', {
                    ...
                    vendorId: user.id || user.email // Support for User ID tracking
                });
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
            ...
            <div className="p-8 max-w-7xl mx-auto mt-6">
                ...
                <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 bg-slate-200 dark:bg-slate-800 p-1 rounded-xl mb-8 max-w-4xl mx-auto md:mx-0">
                    ...
                    <button onClick={() => setMode('logs')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === 'logs' ? 'bg-white dark:bg-slate-700 shadow-sm text-amber-600 dark:text-amber-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>📜 History & Logs</button>
                </div>
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    ...
                                    {myLogs.map((log) => (
                                        <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                            <td className="p-4 font-mono text-xs text-slate-400">{new Date(log.created_at).toLocaleString()}</td>
                                            <td className="p-4 font-bold text-slate-800 dark:text-slate-200">BOQ / Quote</td>
                                            <td className="p-4 text-blue-600 dark:text-blue-400 font-medium">{log.document_number}</td>
                                            <td className="p-4">
                                                <span className={`px-3 py-1 rounded-full text-xs font-black uppercase ${log.status.includes('PO Generated') ? 'bg-emerald-100 text-emerald-700' : log.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                                                    {log.status}
                                                </span>
                                            </td>
                                        </tr>
                ...
                {mode === 'boq' && (
                    <div className="max-w-4xl">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-200 mt-6">
                            ...
                            {/* 🔥 Accept large PDFs and Excels by targeting MIME types in the UI layer */}
