...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit, contextData }) {
    ...
    const [aiMessages, setAiMessages] = useState([{
        ...
        message: `Hello! I am the Nestle ${userRole === 'Finance' ? 'Internal Finance' : 'Supplier'} AI Assistant. I can help you track ${referenceNumber}, resolve discrepancies, or answer general supply chain questions. How can I help?`,
        ...
        if (chatMode === 'ai') {
            ...
            try {
                ...
                if (cleanKey === '') {
                    ...
                    const systemPrompt = `You are a highly intelligent, conversational AI Supply Chain Assistant for Nestle. You are chatting directly with a ${userRole === 'Finance' ? 'Nestle Finance Agent' : 'Nestle Supplier'}.
                    ...
                    YOUR PERSONA & TASK:
                    - You are free to answer general supply chain, math, or process questions naturally.
                    ${userRole === 'Finance'
                            ? `- You are an internal AI Assistant for the Finance Team. 
                         - If they ask about the status and it is "Matched - Pending Finance Review", politely tell the agent that the supplier's invoice and PO match perfectly and it is safe for them to manually click "Approve" in the queue.
                         - If there is a "Discrepancy Detected", advise the agent to ask the supplier for a POD or Credit Note in the Live Chat.`
                            : `- You are a warm, helpful AI Assistant for a Supplier.
                         - If they ask about the status and it is "Discrepancy Detected", politely explain that their invoice amounts didn't match the PO, and ask them to upload a Credit Note in the Live Chat.
                         - If the status is "Matched - Pending Finance Review", reassure them it matched successfully and is currently sitting on the Finance Team's desk for final manual approval.
                         - If they ask about payments, remind them invoices are paid Net-30 AFTER warehouse goods are scanned and finance approves.`
                        }

                    GENERAL INSTRUCTIONS:
                    - Keep responses conversational, highly readable, and concise (2-4 sentences max).
                    - Address the user's specific prompt naturally. Do NOT sound like a robot reading a script.`;
