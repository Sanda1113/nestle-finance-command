    ...
    const handleMatchUpload = async () => {
        ...
        try {
            ...
            if (invRes.data.success && poRes.data.success) {
                ...
                let status = 'Finance Review Pending';
            ...
            <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
                ...
                <div className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800 p-4 px-6 flex flex-wrap justify-between items-center sticky top-0 z-20 shadow-sm">
                    ...
                    <div className="flex items-center gap-4">
                        <button onClick={() => { fetchPOs(); fetchLogs(); }} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors" title="Force Refresh Data">
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {filteredPOs.length === 0 ?
                                            ...
                                            : filteredPOs.map(po => {
                                                ...
                                                // 🛡️ Ensure they submitted an Invoice match before delivering
                            ...
                            {mode === 'match' && (
                                <div className="max-w-3xl animate-in fade-in duration-300">
                                    ...
                                    {matchStatus !== 'Pending' && matchStatus !== 'Submitted' && (
                                        <div className="mt-6">
                                            <div className={`p-3 mb-4 rounded-lg text-sm text-center font-medium border ${matchStatus === 'Finance Review Pending' ? 'bg-emerald-900/30 border-emerald-800 text-emerald-300' : 'bg-amber-900/30 border-amber-800 text-amber-300'}`}>
                                                {matchStatus === 'Finance Review Pending' ? '✅ Finance Review Pending' : '⚠️ Discrepancy Routed for Review'}
                                            </div>
                                            ...
                                            <div className="relative border-l-2 border-slate-800 ml-3 space-y-6 pb-2">
                                                {myLogs.map((log) => {
                                                    ...
                                                    if (log.type.includes('Match') || log.type.includes('Reconciliation')) {
                                                        ...
                                                        if (liveStatus === 'Finance Review Pending' || liveStatus === 'Pending' || liveStatus === 'Processing' || !liveStatus) {
                                                            liveStatus = 'Finance Team Checking';
