...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit, contextData }) {
    ...
    // 🧠 NEW: Dual Mode System (AI vs Human)
    ...
    const quickReplies = userRole === 'Finance'
        ? ["Please upload Proof of Delivery (POD).", "Can you clarify this variance?", "Requesting Credit Note."]
    ...
    useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages, chatMode, aiThinking]);

    const sendMessage = async (text = newMessage, forceRole = userRole, metadata = null) => {
        if (!text.trim() && !metadata) return;
        setIsSending(true);

        // 🤖 IF IN AI MODE: Simulate AI Response instead of saving to DB
        if (chatMode === 'ai' && userRole === 'Supplier') {
            const userMsg = { id: Date.now(), sender_role: 'Supplier', message: text, created_at: new Date().toISOString() };
            ...
            setTimeout(() => {
                ...
                if (lowerText.includes('status') || lowerText.includes('where is my')) {
                    const statusStr = contextData?.status || 'Processing';
                    aiResponse = `According to the system, Shipment/PO ${referenceNumber} is currently: **${statusStr}**.\n\nIf this requires action, please switch to Live Agent mode to speak with Finance.`;
                } else if (lowerText.includes('pay') || lowerText.includes('money')) {
                    aiResponse = `Invoices matched without discrepancies are typically paid on Net-30 terms. Ensure your dock delivery has been signed for by the warehouse to unlock payment processing.`;
                } else {
                    aiResponse = `I am the AI Tracking Assistant. I can check statuses and payment terms. For specific resolution regarding ${referenceNumber}, please switch to 'Live Agent' mode at the top.`;
                }
    ...
    const getStatusStyle = () => {
        ...
        return 'bg-emerald-500'; // Default clear
    };
    ...
    return (
        <div className="flex flex-col h-[650px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden font-sans">
            ...
            {userRole === 'Supplier' && (
                <div className="flex border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900/50">
                    <button
                        ...
                        className={`flex-1 py-2.5 text-xs font-bold flex items-center justify-center gap-2 transition-all ${chatMode === 'human' ? 'bg-white dark:bg-slate-800 text-emerald-600 dark:text-emerald-400 border-b-2 border-emerald-500' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800/80'}`}
                    >
                        <User className="w-4 h-4" /> Live Agent
                    </button>
                    <button
                        onClick={() => setChatMode('ai')}
                        className={`flex-1 py-2.5 text-xs font-bold flex items-center justify-center gap-2 transition-all ${chatMode === 'ai' ? 'bg-white dark:bg-slate-800 text-purple-600 dark:text-purple-400 border-b-2 border-purple-500' : 'text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800/80'}`}
                    >
            ...
            {/* Warning Banner */}
            {userRole === 'Finance' && varianceType && disputeStatus === 'Open' && (
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 bg-slate-50/50 dark:bg-slate-950/50 relative">
                {messages.length === 0 && chatMode === 'human' && (
                ...
                {chatMode === 'ai' && messages.filter(m => m.sender_role === 'AI System' || (m.sender_role === 'Supplier' && !m.id?.toString().includes('-'))).length === 0 && (
                    <div className="flex w-full justify-start">
                        <div className="flex max-w-[85%] gap-3 flex-row">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-md bg-linear-to-br from-purple-500 to-indigo-600 shrink-0">AI</div>
                            <div className="flex flex-col items-start">
                                <span className="text-[10px] font-bold text-slate-400 mb-1.5 px-1 uppercase tracking-wider">AI System</span>
                                <div className="px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-tl-sm">
                                    Hello! I am your tracking assistant for {referenceNumber}. You can ask me things like "What is the status?" or "When will I get paid?"
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {messages.map((msg, idx) => {
                    // Hide AI messages if in human mode, hide human messages if in AI mode
                    if (chatMode === 'human' && msg.sender_role === 'AI System') return null;
                    if (chatMode === 'ai' && msg.sender_role !== 'AI System' && msg.sender_role !== 'Supplier') return null;
                    // If in AI mode, only show supplier messages that were typed *during* AI mode (simulated ones have fresh IDs)
                    if (chatMode === 'ai' && msg.sender_role === 'Supplier' && !msg.id.toString().includes('17')) return null;
                        ...
                        <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
                            <div className={`flex max-w-[85%] gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                    ...
                                    <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed shadow-sm ${isMe ? 'bg-emerald-600 text-white rounded-tr-sm' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-tl-sm'}`}>
                                        {msg.message}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 shrink-0">
                ...
                {userRole === 'Finance' && chatMode === 'human' && (
                    <div className="flex border-b border-slate-100 dark:border-slate-800">
                        <button onClick={() => setIsFormMode(false)} className={`flex-1 py-2 text-xs font-bold ${!isFormMode ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>💬 Quick Chat</button>
                        <button onClick={() => setIsFormMode(true)} className={`flex-1 py-2 text-xs font-bold flex items-center justify-center gap-1 ${isFormMode ? 'text-red-600 border-b-2 border-red-600' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}><FileSpreadsheet className="w-3 h-3" /> Formal Dispute Form</button>
                    </div>
                )}
                    ...
                    <>
                        {userRole === 'Finance' && chatMode === 'human' && disputeStatus !== 'Resolved' && (
                            <div className="px-4 py-3 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 flex flex-wrap gap-2">
                                ...
                                <button onClick={() => handleFormalAction('Issue Credit Note')} className="flex-1 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-blue-500 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-md transition-all shadow-sm">Credit Note</button>
                                <button onClick={() => handleFormalAction('Force Auto-Approve')} className="flex-1 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-emerald-500 text-emerald-600 dark:text-emerald-400 text-xs font-bold rounded-md transition-all shadow-sm">Force Approve</button>
                                <button onClick={() => handleFormalAction('Escalate to Procurement')} className="flex-1 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-red-500 text-red-600 dark:text-red-400 text-xs font-bold rounded-md transition-all shadow-sm">Escalate</button>
                            </div>
                        )}
                        ...
                        <div className="p-4 pt-2">
                            <div className={`flex items-end gap-2 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl border focus-within:ring-2 transition-all ${chatMode === 'ai' ? 'border-purple-200 dark:border-purple-900/50 focus-within:border-purple-500 focus-within:ring-purple-500/20' : 'border-slate-200 dark:border-slate-700 focus-within:border-emerald-500 focus-within:ring-emerald-500/20'}`}>
                                ...
                                />
                                <button onClick={() => sendMessage(newMessage)} disabled={!newMessage.trim() || isSending || aiThinking} className={`p-3 text-white rounded-xl transition-all disabled:opacity-50 flex items-center justify-center shrink-0 shadow-md ${chatMode === 'ai' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
