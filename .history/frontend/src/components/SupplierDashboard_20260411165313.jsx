...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const totalMatched = myLogs.filter(log => (log.status || '').toLowerCase().includes('approve')).length;
    ...
    // 🛡️ STRICT LOG NORMALIZER TO PREVENT "INVALID DATE" AND "SYSTEM ACTION ()" CRASHES
    const normalizedLogs = useMemo(() => {
        return myLogs.map(log => ({
            id: log.id || Math.random().toString(),
            action: log.action || log.message || log.title || 'System Update',
            type: log.type || log.document_type || log.sender_role || 'Record',
            ref: log.ref || log.reference_number || log.po_number || log.document_number || 'General Activity',
            date: log.created_at || log.date || log.timestamp || new Date().toISOString(),
            status: log.status || log.match_status || log.state || 'Processed'
        }));
    }, [myLogs]);
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {filteredPOs.length === 0 ?
                                            ...
                                            : filteredPOs.map(po => {
                                                ...
                                                const isFinanceApproved = relatedRecon && ((relatedRecon.match_status || '').toLowerCase().includes('approve') || (relatedRecon.displayStatus || '').toLowerCase().includes('approve'));
                                                ...
                                                <div className="p-5">
                                                    <div className="relative border-l-2 border-slate-800 ml-3 space-y-6 pb-2">
                                                        {logsForDeal.map((log) => {
                                                            ...
                                                            if (logType.includes('Match') || logType.includes('Reconciliation') || logType.includes('Verification')) {
                                                                ...
                                                                <div key={log.id} className="relative pl-5">
                                                                    ...
                                                                    <div className="flex flex-col">
                                                                        <div className="flex justify-between items-start">
                                                                            <div>
                                                                                <h3 className="text-sm font-bold text-slate-100">{log.action || 'System Action'} <span className="text-slate-400 font-normal">({logType})</span></h3>
                                                                            </div>
                                                                        ...
                                                                        {expandedLog === log.id && (
                                                                            <div className="mt-4 border-t border-slate-800 pt-4">
                                                                                <DisputeChat
                                                                                    ...
                                                                                    contextData={{ status: liveStatus, type: logType, action: log.action }}
                                                                                />
                                                                            </div>
                                                                        )}
                                                                    </div>
