...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    // 🚀 Background Silent Fetching for 1-second real-time updates
        ...
        const html = `
            ...
            <head>
                ...
                <style>
                    ...
                    .letterhead p { margin: 5px 0 0 0; color: #64748b; font-size: 14px; font-weight: bold; text-transform: uppercase; tracking: widest;}
            ...
            <body>
                <div class="letterhead">
                    ...
                    <p style="font-weight:normal; font-size:12px;">123 Corporate Blvd, Colombo, Sri Lanka</p>
                </div>
                <div class="header">
                    <div>
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
            ...
            <div className="p-8 max-w-7xl mx-auto mt-6">
                ...
                <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-2 bg-slate-200 dark:bg-slate-800 p-1 rounded-xl mb-8 max-w-4xl mx-auto md:mx-0">
                    ...
                    <button onClick={() => { setMode('logs'); setMatchStatus('Pending'); setError(null); }} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === 'logs' ? 'bg-white dark:bg-slate-700 shadow-sm text-amber-600 dark:text-amber-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>📜 History & Timeline</button>
                </div>
                            ...
                            {myPOs.length === 0 ? (
                                ...
                                myPOs.map(po => (
                                    <div key={po.id} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border-t-4 border-purple-500 p-6 flex flex-col justify-between">
                                        ...
                                        <button onClick={() => handlePrintPO(po)} className={`w-full py-3 font-bold rounded-xl transition-colors ${po.is_downloaded ? 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-300' : 'bg-purple-600 hover:bg-purple-500 text-white'}`}>
                                            {po.is_downloaded ? "✅ Done (Download Again)" : "📄 Download PDF"}
                                        </button>
                                    </div>
                                ))
                            )}
                ...
                {mode === 'logs' && (
                    <div>
                        <h2 className="text-4xl font-black text-slate-800 dark:text-white mb-2">Live Lifecycle Timeline</h2>
                        <p className="text-slate-500 dark:text-slate-400 mb-6">Real-time tracking from BOQ submission to Payment processing.</p>

                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                            {myLogs.length === 0 ? <p className="text-center text-slate-500 py-8">No activity logged yet.</p> : (
                                <div className="relative border-l-2 border-slate-200 dark:border-slate-700 ml-4 space-y-8 pb-4">
                                    {myLogs.map((log, idx) => (
                                        <div key={log.id} className="relative pl-6">
                                            <div className={`absolute w-4 h-4 rounded-full -left-[9px] top-1 border-4 border-white dark:border-slate-900 ${log.status?.includes('Approve') || log.status?.includes('Generated') || log.status === 'Downloaded' ? 'bg-emerald-500' : log.status?.includes('Reject') || log.status?.includes('Discrepancy') ? 'bg-red-500' : 'bg-blue-500'}`}></div>

                                            <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-1">
                                                <div>
                                                    <h3 className="font-black text-lg text-slate-800 dark:text-white">{log.action}</h3>
                                                    <p className="text-sm font-bold text-slate-500 mb-2">{log.type}: <span className="text-blue-600 dark:text-blue-400">{log.ref}</span></p>
                                                </div>
                                                <div className="text-left md:text-right">
                                                    ...
                                                    <p className="text-xs text-slate-400 mt-2 font-mono">{new Date(log.date).toLocaleString()}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                ...
                {mode === 'boq' && (
                    <div className="max-w-4xl">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-200 mt-6">
                            ...
                            {/* 🚀 Supported Files Include Excel & CSV natively */}
                            <input type="file" accept=".pdf, image/*, .xlsx, .xls, .csv" onChange={(e) => setBoqFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-blue-50 file:text-blue-700 cursor-pointer" />
                    ...
                    <div className="max-w-4xl">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-200 flex flex-col md:flex-row gap-6 mt-6">
                            <div className="flex-1">
                                ...
                                <input type="file" accept=".pdf, image/*, .xlsx, .xls, .csv" onChange={(e) => setInvoiceFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-blue-50 file:text-blue-700 cursor-pointer" />
                            </div>
                            <div className="flex-1">
                                <label className="block text-sm font-bold text-slate-500 uppercase mb-3">2. Select PO</label>
                                <input type="file" accept=".pdf, image/*, .xlsx, .xls, .csv" onChange={(e) => setPoFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-6 file:rounded-xl file:border-0 file:bg-purple-50 file:text-purple-700 cursor-pointer" />
                            </div>
                        </div>
