...
export default function DisputeChat({ referenceNumber, userEmail, userRole, varianceType, onResubmit, contextData }) {
    ...
    const [aiMessages, setAiMessages] = useState([{
        ...
        message: `Hello! I am your AI tracking assistant for ${referenceNumber}. You can ask me things like "What is the status?" or "When will I get paid?"`,
    ...
    const sendMessage = async (text = newMessage, forceRole = userRole, metadata = null) => {
        ...
        // ==========================================
        // 🤖 AI MODE LOGIC (DEEPSEEK API CALL)
        // ==========================================
        if (chatMode === 'ai') {
            ...
            try {
                let aiResponseText = "";
                let actualDocumentStatus = contextData?.status || 'Awaiting System Update';
                if (messages.some(m => m.message.toLowerCase().includes('reject'))) {
                    actualDocumentStatus = 'Rejected';
                }

                const cleanKey = DEEPSEEK_API_KEY.trim();

                if (cleanKey === '') {
                    aiResponseText = `⚠️ API Key is missing.`;
                } else {
                    ...
                    1. BOQ (Quote) -> Submitted by Supplier. Finance reviews it. Can be "Rejected" or "Approved".
                    2. Invoice Match -> Submitted by Supplier. Status becomes "Finance Review Pending".
                    3. Finance Approval -> Finance approves docs. Status becomes "Approved by Finance (Awaiting Delivery)".
                    4. Dock Delivery -> Supplier marks "Delivered". Status becomes "Pending Warehouse GRN".
                    5. Payout -> Warehouse scans items. Status becomes "Approved - Awaiting Payout".

                    INSTRUCTIONS FOR YOUR RESPONSE:
                    1. Answer the user's query STRICTLY based on the "Current System Status" provided above.
                    2. IF THE STATUS SAYS 'Rejected', explicitly state it has been rejected and advise checking the Live Agent chat.
                    3. IF THE STATUS SAYS 'Finance Review Pending', state that the documents are currently being manually verified by the Nestle Finance team.
                    4. If they ask about payments, tell them invoices are only paid on Net-30 terms AFTER the Warehouse GRN is complete and Finance gives final Approval.
                ...
                const errorMsg = {
                    id: Date.now() + 1,
                    sender_role: 'AI System',
                    message: "⚠️ AI Server is temporarily unreachable. Please check your internet connection or switch to Live Agent mode.",
                    created_at: new Date().toISOString()
                };
    ...
    const exportAuditPDF = () => {
        ...
        const modeLabel = chatMode === 'ai' ? 'AI Tracking Assistant' : 'Live Agent Audit';
        ...
        <div className="flex flex-col h-[650px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden font-sans">
            ...
            <div className="px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center relative overflow-hidden shrink-0">
                ...
                <div className="flex gap-2">
                    <button onClick={exportAuditPDF} className="flex items-center gap-1.5 px-3 py-1.5 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 text-xs font-bold rounded-lg transition-colors" title="Export Chat Log">
