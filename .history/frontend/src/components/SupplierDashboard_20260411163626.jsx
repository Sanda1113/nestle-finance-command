...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    useEffect(() => {
        if (mode === 'inbox') fetchData();
        if (mode === 'logs') fetchData();
    ...
    // 🛡️ STRICT LOG NORMALIZER TO PREVENT "INVALID DATE" CRASHES
    const normalizedLogs = useMemo(() => {
        return myLogs.map(log => ({
            id: log.id || Math.random().toString(),
            action: log.action || log.message || 'Status Update',
            type: log.type || log.sender_role || 'System',
            ref: log.ref || log.reference_number || log.po_number || 'General',
            date: log.date || log.created_at || new Date().toISOString(),
            status: log.status || log.match_status || 'Pending'
        }));
    }, [myLogs]);
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {filteredPOs.length === 0 ?
                                            ...
                                            : filteredPOs.map(po => {
                                                ...
                                                // 🛡️ Ensure they submitted an Invoice match before delivering AND Finance Approved it
                                                const relatedRecon = myRecons.find(r => {
                                                    const poNumMatch = (po.po_number || '').match(/\d+/);
                                                    const poNumOnly = poNumMatch ? poNumMatch[0] : po.po_number;
                                                    return (r.po_number || '').includes(poNumOnly) || (r.invoice_number || '').includes(poNumOnly);
                                                });

                                                const isFinanceApproved = relatedRecon && ((relatedRecon.match_status || '') === 'Approved' || (relatedRecon.displayStatus || '').includes('Approved'));
                                        ...
                                        {recentLogs.map(log => (
                                            <div key={log.id} className="flex items-start gap-2 text-sm border-b border-slate-800 pb-2 last:border-0">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-[10px] text-slate-400">{new Date(log.date || log.created_at || new Date()).toLocaleString()}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
