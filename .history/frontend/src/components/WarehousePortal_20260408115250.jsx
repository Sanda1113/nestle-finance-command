...
import { Package, Truck, CheckCircle2, AlertCircle, ScanBarcode, ArrowLeft, Camera, Search, EyeOff, Eye, CalendarDays, Hash, ShieldAlert, ShieldCheck, WifiOff, MapPin, RefreshCw, LogOut, Moon, Sun, X } from 'lucide-react';
...
export default function WarehousePortal({ user, onLogout }) {
    ...
    // 🚀 NEW: Ref to hold latest state so camera doesn't constantly restart
    const latestState = useRef({ pos, selectedPO, receivedItems });
    useEffect(() => {
        latestState.current = { pos, selectedPO, receivedItems };
    }, [pos, selectedPO, receivedItems]);
    ...
    useEffect(() => {
        ...
        // Timeout ensures the <div id="reader"> is painted on screen before library attaches to it
            ...
            html5QrCode.start(
                ...
                (decodedText) => {
                    ...
                    html5QrCode.stop().then(() => {
                        ...
                        if (!state.selectedPO) {
                            // 1. Scanning from Dashboard -> Look for a PO
                            const matchedPO = state.pos.find(p => p.po_number.toLowerCase() === decodedText.toLowerCase());
                            if (matchedPO) {
                                setSearchTerm(decodedText);
                                handleSelectPO(matchedPO);
                            } else {
                                alert(`Scanned: ${decodedText}\nNo matching PO found in the dock queue.`);
                            }
                        } else {
                            // 2. Scanning inside a PO -> Add +1 to item quantity
                            // For demo purposes, we will add +1 to the first item. 
                            // In a real app, you would match decodedText to the item's SKU/Barcode field.
                            handleQtyChange(0, 1);
                            alert(`✅ Barcode Scanned!\n+1 Unit added to ${state.receivedItems[0].description}`);
                        }
                    }).catch(console.error);
                },
                (errorMessage) => { /* Background scan errors, ignore */ }
            ).catch(err => {
                console.error("Camera start error:", err);
                alert("Could not start camera. Please ensure you are using HTTPS and have granted browser camera permissions.");
                setScanning(false);
            });
        }, 100);
    ...
    const handleQtyChange = (index, delta, exactVal = null) => {
        // Use latest state ref to ensure accurate updates from scanner callback
        ...
        if (newVal === 0) { currentItems[index].status = 'Pending Scan'; currentItems[index].riskLevel = 'Low'; }
            ...
            currentItems[index].riskLevel = shortagePercent > 20 || selectedPO.trustScore < 80 ? 'High' : 'Medium';
        }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans flex flex-col transition-colors duration-300 relative">
            ...
            {scanning && (
                <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex flex-col items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-300">
                    <button onClick={() => setScanning(false)} className="absolute top-6 right-6 bg-slate-800 text-white p-3 rounded-full hover:bg-slate-700 transition-colors shadow-lg">
                        <X className="w-6 h-6" />
                    </button>

                    <div className="flex flex-col items-center mb-6">
                        <ScanBarcode className="w-12 h-12 text-blue-500 mb-2 animate-pulse" />
                        <h2 className="text-white font-black text-2xl tracking-tight text-center">
                            {selectedPO ? 'Scan Product Barcode' : 'Scan PO Barcode'}
                        </h2>
                        <p className="text-slate-400 text-sm mt-1 text-center">Align the barcode within the frame</p>
                    </div>

                    <div className="w-full max-w-sm bg-black rounded-3xl overflow-hidden border-2 border-slate-700 relative shadow-[0_0_50px_rgba(59,130,246,0.15)]">
                        <div className="absolute top-4 left-4 bg-red-500 text-white text-[10px] font-black uppercase px-2 py-1 rounded shadow-md z-10 flex items-center gap-1.5">
                            <div className="w-2 h-2 rounded-full bg-white animate-pulse"></div> Camera Active
                        </div>

                        {/* The HTML5 QR Code library injects the video here */}
                        <div id="reader" className="w-full h-full min-h-[300px]"></div>

                        {/* Scanning Laser Animation */}
                        <div className="absolute top-0 left-0 w-full h-[2px] bg-red-500 shadow-[0_0_20px_rgba(239,68,68,1)] animate-[scan_2.5s_ease-in-out_infinite]"></div>
                    </div>
                </div>
            )}
            ...
            <style>{`
                ...
                /* Hide the ugly default borders of html5-qrcode */
                #reader img, #reader video { border-radius: 1.5rem; object-fit: cover; }
                #reader__dashboard_section_csr span { color: white !important; }
                #reader__dashboard_section_swaplink { color: #3b82f6 !important; margin-top: 10px; display: inline-block; }
            `}</style>
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    {isOffline ? (
                        <span className="flex items-center gap-1 bg-red-500/20 text-red-400 border border-red-500/30 px-2.5 py-1 rounded-full text-[10px] md:text-xs font-bold animate-pulse">
                            <WifiOff className="w-3 h-3 shrink-0" /> <span className="hidden sm:inline">Offline (Local Storage)</span><span className="sm:hidden">Offline</span>
                        </span>
                    ) : (
                ...
                {loading ? (
                    ...
                    <div className="space-y-4 md:space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 max-w-2xl">
                            ...
                            <button onClick={() => setScanning(true)} className={`w-full sm:w-auto px-5 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all bg-slate-800 dark:bg-slate-800 text-white hover:bg-slate-700 shadow-md`}>
                                <Camera className="w-5 h-5 shrink-0" /> Open Camera Scanner
                            </button>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
                            {filteredPOs.map(po => (
                                <div key={po.id} onClick={() => handleSelectPO(po)} className="group bg-white dark:bg-slate-900 p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl cursor-pointer transition-all relative overflow-hidden">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 flex flex-col lg:flex-row overflow-hidden relative">
                        ...
                        <div className="lg:w-1/3 bg-slate-50 dark:bg-slate-800/30 p-5 md:p-6 lg:p-8 border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800 flex flex-col">
                            ...
                            {/* Scan Item Action */}
                            <button onClick={() => setScanning(true)} className="w-full mb-3 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all shadow-md flex items-center justify-center gap-2">
                                <ScanBarcode className="w-5 h-5" /> Camera Scan Items
                            </button>
                                    ...
                                    return (
                                        <div key={idx} className={`p-4 md:p-5 rounded-2xl border-2 transition-all bg-white dark:bg-slate-950 ${isHighRisk ? 'border-red-500 shadow-lg shadow-red-500/20' : isShort ? 'border-red-500/50' : isOver ? 'border-amber-500/50' : isPending ? 'border-slate-200 dark:border-slate-800' : 'border-emerald-500/30'}`}>
                                            ...
                                            <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                                                ...
                                                {/* Thumb-Friendly Mobile Stepper (Full Width on Mobile) */}
                                            ...
                                            {/* FMCG Tracking (Stacks on Mobile) */}
                        ...
                        {/* Mobile Fixed Sticky Footer for Submission */}
