...
function FinancePortal({ user }) {
    ...
    const enrichedRecords = records.map(r => {
        ...
        const isMathMatch = Math.abs((r.invoice_total || 0) - (r.po_total || 0)) <= 0.01;

        // 🛡️ STRICT MATH RECALCULATION
        if (!displayStatus || displayStatus === 'Pending' || displayStatus.includes('Manual Review')) {
            displayStatus = isMathMatch ? 'Matched - Pending Finance Review' : 'Discrepancy Detected';
        }

        // Apply Lifecycle Overrides if already approved
        if (displayStatus.includes('Approve')) {
