...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    const [myRecons, setMyRecons] = useState([]); // True match status data source
    ...
    const fetchData = async () => {
        try {
            ...
            // Sort POs explicitly newest first
            const sortedPOs = (posRes.data.data || []).sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));

            setMyPOs(sortedPOs);
            setMyLogs(logsRes.data.logs || []);
            setMyRecons((reconsRes.data.data || []).filter(r => r.supplier_email === user.email));
    ...
    const handleMatchUpload = async () => {
        ...
        try {
            ...
            if (invRes.data.success && poRes.data.success) {
                ...
                const safeParse = (val) => Number(val?.toString().replace(/[^0-9.-]+/g, "")) || 0;
    ...
    const groupedLogs = useMemo(() => {
        const groups = {};
        myLogs.forEach(log => {
            const ref = log.ref || 'General';
            const match = ref.match(/\d+/);
            const transactionId = match ? match[0] : ref;

            if (!groups[transactionId]) groups[transactionId] = [];
            groups[transactionId].push(log);
        });

        // Sort logs within their groups by date (oldest to newest)
        Object.keys(groups).forEach(key => {
            groups[key].sort((a, b) => new Date(a.date) - new Date(b.date));
        });

        // Sort the entire group container by the newest overall activity
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {filteredPOs.length === 0 ?
                                            ...
                                            : filteredPOs.map(po => {
                                                ...
                                                const poNumeric = po.po_number?.match(/\d+/)?.[0];
                                                const relatedRecon = myRecons.find(r => r.po_number?.includes(poNumeric) || r.invoice_number?.includes(poNumeric));
                                                const hasSubmittedDocs = !!relatedRecon;
                                                            ...
                                                            <div className="flex flex-wrap gap-2 mt-4">
                                                                ...
                                                                {!isDelivered ? (
                                                                    <button
                                                                        ...
                                                                        disabled={!hasSubmittedDocs}
                                                                        className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${hasSubmittedDocs ? 'bg-amber-600 hover:bg-amber-500 text-white shadow-sm border border-amber-500' : 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'}`}
                                                                        title={!hasSubmittedDocs ? "Submit Invoice & PO first" : "Mark as arriving at Dock"}
                                                                    >
                                                                        🚚 {hasSubmittedDocs ? 'Mark Delivered' : 'Submit Docs First'}
                                                                    </button>
                                                                ) : (
                                                ...
                                                <div className="p-5">
                                                    <div className="relative border-l-2 border-slate-800 ml-3 space-y-6 pb-2">
                                                        {logsForDeal.map((log) => {
                                                            ...
                                                            const numericRef = log.ref?.match(/\d+/)?.[0] || log.ref;

                                                            // 🛡️ STRICT MATH RECALCULATION & LIFECYCLE OVERRIDES
                                                            if (log.type.includes('Match') || log.type.includes('Reconciliation') || log.type.includes('Verification')) {
                                                                const relatedRecon = myRecons.find(r => r.po_number?.includes(numericRef) || r.invoice_number?.includes(numericRef));

                                                                if (relatedRecon) {
                                                                    const isMathMatch = Math.abs((relatedRecon.invoice_total || 0) - (relatedRecon.po_total || 0)) <= 0.01;
                                                                    liveStatus = relatedRecon.match_status;

                                                                    if (!liveStatus || liveStatus === 'Pending' || liveStatus.includes('Manual Review')) {
                                                                        liveStatus = isMathMatch ? 'Matched - Pending Finance Review' : 'Discrepancy Detected';
                                                                    }

                                                                    const relatedPO = myPOs.find(p => p.po_number?.includes(numericRef));
                                                                    ...
                                                                    if (liveStatus.includes('Approve')) {
                                                            ...
                                                            if (log.type.includes('BOQ')) {
                                                                const rejectLog = logsForDeal.find(l => l.status.includes('Reject'));
