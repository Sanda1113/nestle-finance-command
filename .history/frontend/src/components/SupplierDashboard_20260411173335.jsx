...
export default function SupplierDashboard({ user, onLogout }) {
    ...
    // 🛡️ BULLETPROOF LOG NORMALIZER TO PREVENT "INVALID DATE" AND "SYSTEM ACTION ()" CRASHES
    ...
    // 🛡️ RECENT LOGS RESTORED - Safely sorted using the normalized array to fix the ReferenceError
    ...
    // 🗂️ GROUP LOGS BY TRANSACTION NUMBER
    const groupedLogs = useMemo(() => {
        const groups = {};
        normalizedLogs.forEach(log => {
            const match = log.ref.match(/\d+/);
            const transactionId = match ? match[0] : log.ref;

            if (!groups[transactionId]) groups[transactionId] = [];
        ...
        return Object.entries(groups).sort((a, b) => {
            const latestA = Math.max(...a[1].map(l => new Date(l.date).getTime()));
            const latestB = Math.max(...b[1].map(l => new Date(l.date).getTime()));
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {filteredPOs.length === 0 ?
                                            ...
                                            : filteredPOs.map(po => {
                                                ...
                                                // Delivery requires manual Finance Approval first
                            ...
                            {mode === 'logs' && (
                                <div className="animate-in fade-in duration-300">
                                    <div className="mb-4">
                                        ...
                                        <p className="text-sm text-slate-400">Activity grouped by Transaction Number.</p>
                                    </div>
                                    ...
                                    {Object.keys(groupedLogs).length === 0 ? (
                                        ...
                                        Object.entries(groupedLogs).map(([transactionId, logsForDeal]) => (
                                            <div key={transactionId} className="bg-slate-900 rounded-xl shadow-sm border border-slate-800 overflow-hidden mb-6">
                                                <div className="bg-slate-800/80 px-4 py-3 border-b border-slate-700 flex justify-between items-center">
                                                    <h3 className="font-bold text-slate-200 flex items-center gap-2">
                                                        <Tag className="w-4 h-4 text-blue-400" /> Transaction Cycle: #{transactionId}
                                                    </h3>
                                                </div>
                                                <div className="p-5">
                                                    <div className="relative border-l-2 border-slate-800 ml-3 space-y-6 pb-2">
                                                        {logsForDeal.map((log) => {
                                                            ...
                                                            const numericRef = String(log.ref || '').match(/\d+/)?.[0] || String(log.ref);

                                                            // 🛡️ STRICT MATH RECALCULATION & LIFECYCLE OVERRIDES
                                                            if (logType.includes('match') || logType.includes('reconciliation') || logType.includes('verif') || logAction.includes('verif') || logAction.includes('match')) {
                                                                const relatedRecon = myRecons.find(r => String(r.po_number || '').includes(numericRef) || String(r.invoice_number || '').includes(numericRef));

                                                                if (relatedRecon) {
                                                                    const invTotal = safeParse(relatedRecon.invoice_total);
                                                                    const poTotal = safeParse(relatedRecon.po_total);
                                                                    const isMathMatch = Math.abs(invTotal - poTotal) <= 0.01;

                                                                    liveStatus = String(relatedRecon.match_status || '');

                                                                    if (!liveStatus || liveStatus === 'Pending' || liveStatus.includes('Manual Review') || liveStatus.includes('Discrepancy')) {
                                                                        liveStatus = isMathMatch ? 'Matched - Pending Finance Review' : 'Discrepancy Detected';
                                                                    }

                                                                    const relatedPO = myPOs.find(p => String(p.po_number || '').includes(numericRef));
                                                                    const isReceived = relatedPO && String(relatedPO.status || '').includes('Received');
                                                                    const isDelivered = relatedPO && (relatedPO.status === 'Delivered to Dock' || relatedPO.po_data?.delivery_timestamp);

                                                                    if (liveStatus.toLowerCase().includes('approve')) {
                                                                        if (isReceived) liveStatus = 'Approved - Awaiting Payout';
                                                                        else if (isDelivered) liveStatus = 'Pending Warehouse GRN';
                                                                        else liveStatus = 'Approved by Finance (Awaiting Delivery)';
                                                                    }
                                                                }
                                                            }

                                                            if (logType.includes('boq')) {
                                                                const rejectLog = logsForDeal.find(l => String(l.status || '').includes('Reject'));
                                                                if (rejectLog) liveStatus = 'Rejected';
                                                            }

                                                            const hasDispute = liveStatus.toLowerCase().includes('reject') || liveStatus.toLowerCase().includes('discrepancy');
                                                            const isMatched = liveStatus === 'Matched - Pending Finance Review';

                                                            return (
                                                                <div key={log.id} className="relative pl-5">
                                                                    <div className={`absolute w-3 h-3 rounded-full -left-[7px] top-1.5 ring-4 ring-slate-900 ${liveStatus.toLowerCase().includes('approve') || liveStatus.toLowerCase().includes('generated') || liveStatus.toLowerCase() === 'downloaded' ? 'bg-emerald-500' :
                                                                        hasDispute ? 'bg-red-500' : isMatched ? 'bg-blue-500' : 'bg-blue-500'
                                                                        }`}></div>
                                                                    <div className="flex flex-col">
                                                                        <div className="flex justify-between items-start">
                                                                            <div>
                                                                                <h3 className="text-sm font-bold text-slate-100">{log.action || 'System Action'} <span className="text-slate-400 font-normal">({log.type || 'System'})</span></h3>
                                                                            </div>
                                                                            <div className="text-right flex flex-col items-end gap-2">
                                                                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ring-1 ring-inset ${liveStatus.toLowerCase().includes('approve') || liveStatus.toLowerCase().includes('generated') || liveStatus.toLowerCase() === 'downloaded' ? 'bg-emerald-900/50 text-emerald-400 ring-emerald-500/20' :
                                                                                    hasDispute ? 'bg-red-900/50 text-red-400 ring-red-500/20' : 'bg-blue-900/50 text-blue-400 ring-blue-500/20'
                                                                                    }`}>{liveStatus}</span>
                                                                                ...
                                                                                <p className="text-[10px] text-slate-400">{safeDate(log.date)}</p>
                                                                            </div>
                                                                        </div>
