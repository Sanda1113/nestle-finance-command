...
export default function AppNotifier({ role }) {
    ...
    const fetchNotifications = async () => {
        try {
            const res = await axios.get(`https://nestle-finance-command-production.up.railway.app/api/sprint2/notifications/${role}`);
            ...
            if (newNotifications.length > 0) {
                ...
                // Mark them as read in the DB so they don't fire again
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[999] flex flex-col gap-3 pointer-events-none">
            {toasts.map(toast => (
                <div key={toast.id} className="bg-white dark:bg-slate-900 border-l-4 border-blue-500 rounded-lg shadow-2xl p-4 w-80 flex items-start gap-3 animate-in slide-in-from-right-8 pointer-events-auto transition-all">
                    ...
                    <div className="flex-1">
                        <h4 className="text-sm font-black text-slate-800 dark:text-white leading-tight">System Alert</h4>
