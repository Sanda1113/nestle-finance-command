...
router.post('/grn/submit', async (req, res) => {
    ...
    try {
        const { error: grnErr } = await supabase.from('grns').insert([{
            ...
            // If you added a gps_location column to your DB, it would save here.
            // For now, we will rely on the frontend passing it.
        }]);
...
router.post('/disputes/send', async (req, res) => {
    const { referenceNumber, senderEmail, senderRole, message, metadata } = req.body; // Added metadata for structured forms
    try {
        // If metadata exists, append it to the message for rendering
