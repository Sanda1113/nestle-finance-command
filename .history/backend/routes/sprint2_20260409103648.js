...
router.get('/grn/pending-pos', async (req, res) => {
    try {
        ...
        // 🔹 DYNAMIC SHIPMENT ID INJECTION (Since we didn't alter the DB schema)
        // We hash the PO ID to create a stable, unique 6-character Shipment ID
        const enhancedData = data.map(po => {
            const hash = Math.abs(po.id * 12345).toString(16).toUpperCase().substring(0, 6).padStart(6, '0');
            return {
                ...po,
