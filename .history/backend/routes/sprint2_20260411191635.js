...
router.post('/disputes/send', async (req, res) => {
    ...
    try {
        ...
        // 🔔 IF FINANCE SENDS A MESSAGE, EMAIL THE SUPPLIER.
        if (senderRole === 'Finance') {
            // Find the PO (or BOQ) associated with this reference number
            // The referenceNumber is typically the PO number or invoice number
            ...
            // If not found, try looking in BOQs (in case this is a BOQ conversation)
            if (!supplierEmail) {
            ...
            if (supplierEmail) {
                ...
                console.warn(`⚠️ Could not find supplier email for reference: ${referenceNumber}`);
            }
