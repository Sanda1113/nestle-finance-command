...
app.post('/api/extract-invoice', upload.single('invoiceFile'), async (req, res) => {
    try {
        ...
        // ... (getSafeString, getAddressText, getNum, extractCurrency, extractLineItems, getRefNumbers remain unchanged) ...
...
app.patch('/api/reconciliations/:id', async (req, res) => {
    ...
    try {
        ...
        // Fetch reconciliation to get supplier_email
        const { data: recon, error: fetchErr } = await supabase.from('reconciliations').select('supplier_email, invoice_number').eq('id', id).single();
        if (fetchErr) {
            ...
            sendSupplierEmail(
                ...
                `Invoice ${statusText}`,
                `<h3>Invoice ${statusText}</h3><p>Your invoice ${recon.invoice_number} has been ${statusText} by Finance.</p>`
...
app.post('/api/boqs/:id/reject', async (req, res) => {
    ...
    try {
        ...
        const { data: boq, error: fetchErr } = await supabase.from('boqs').select('supplier_email, vendor_name').eq('id', id).single();
        if (fetchErr) {
            ...
            sendSupplierEmail(
                boq.supplier_email,
                `BOQ Rejected`,
                `<h3>Quote Rejected</h3><p>Dear ${boq.vendor_name},</p><p>Your quote was rejected. Reason: ${reason}</p>`
...
app.post('/api/boqs/:id/generate-po', async (req, res) => {
    ...
    try {
        ...
        if (boqData.supplier_email) {
            sendSupplierEmail(
                boqData.supplier_email,
                `PO Generated – ${generatedPoNumber}`,
                `<h3>Purchase Order Generated</h3><p>Dear ${boqData.vendor_name},</p><p>Your quote has been approved and PO ${generatedPoNumber} created.</p>`
...
app.get('/api/supplier/logs/:email', async (req, res) => {
    ...
    try {
        ...
        const { data: boqs, error: boqErr } = await supabase.from('boqs').select('id, document_number, total_amount, status, created_at, vendor_name').eq('supplier_email', email);
...
process.on('uncaughtException', (error) => {
    ...
    // Keep the process alive, but log prominently
});
