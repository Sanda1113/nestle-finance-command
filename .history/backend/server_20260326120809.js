...
app.post('/api/save-boq', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('boqs').insert([{
            ...
            // If Mindee doesn't find a doc number on a quote, generate a temporary one
            ...
            line_items: boqData.lineItems // Supabase natively handles JSONB arrays
        }]);
...
// 3. 1-Click PO Generator
app.post('/api/boqs/:id/generate-po', async (req, res) => {
    const { id } = req.params;
    try {
        // 1. Update the BOQ status
        const { data: boqData, error: updateErr } = await supabase
            .from('boqs')
            .update({ status: 'PO Generated' })
            .eq('id', id)
            .select()
            .single();
        ...
        // 2. Auto-inject the new PO into the purchase_orders table so the system expects it!
        ...
        const { error: poErr } = await supabase.from('purchase_orders').insert([{
            ...
            status: 'Awaiting Invoice' // New status letting the system know it's ready
        }]);

        if (poErr) console.error('Failed to create mock PO in db:', poErr);
