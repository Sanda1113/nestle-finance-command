...
const xlsx = require('xlsx'); // 🚀 NEW: Added Excel parsing library
...
app.post('/api/extract-invoice', upload.single('invoiceFile'), async (req, res) => {
    try {
        ...
        // 🚀 NEW: NATIVE EXCEL & CSV HANDLER (Bypasses Mindee for raw tabular data)
        if (fileName.endsWith('.csv') || fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
            ...
            data.forEach((row, index) => {
                ...
                if (values.length >= 2) {
                    // Assume longest string is description, numbers are qty/price
            ...
            return res.json({
                success: true,
                extractedData: {
                    vendorName: 'Data Upload (Excel/CSV)',
...
// NEW: Helper route for Finance to fetch a specific BOQ by PO Number
app.get('/api/boqs/by-po/:poNumber', async (req, res) => {
    try {
        const { data, error } = await supabase.from('boqs').select('*').like('status', `%${req.params.poNumber}%`).limit(1);
        if (error) throw error;
        res.json({ success: true, data: data[0] || null });
    } catch (error) {
        res.status(500).json({ error: 'Failed to fetch related BOQ' });
    }
});
