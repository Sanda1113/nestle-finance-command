...
app.patch('/api/reconciliations/:id', async (req, res) => {
    ...
    try {
        ...
        const { data: recon } = await supabase.from('reconciliations').select('supplier_email, invoice_number').eq('id', id).single();
        if (recon?.supplier_email) {
            ...
            // In-app notification (via sprint2 notification table)
...
app.post('/api/boqs/:id/reject', async (req, res) => {
    ...
    try {
        ...
        const { data: boq } = await supabase.from('boqs').select('supplier_email, vendor_name').eq('id', id).single();
