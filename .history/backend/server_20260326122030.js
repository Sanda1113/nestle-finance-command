...
app.post('/api/save-boq', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ error: 'Database save failed' }); }
});
...
app.get('/api/boqs', async (req, res) => {
    try {
        ...
        if (error) throw error;
        res.json({ success: true, data });
    } catch (error) { res.status(500).json({ error: 'Failed to fetch BOQs' }); }
});
...
app.post('/api/boqs/:id/generate-po', async (req, res) => {
    ...
    try {
        ...
        // 1. Update the BOQ status with the official PO number so the supplier inbox can read it
        const { error: updateErr } = await supabase
            .from('boqs')
            .update({ status: `PO_GENERATED:${generatedPoNumber}` })
            .eq('id', id);

        if (updateErr) throw updateErr;

        // 2. Auto-inject the new PO into the main database so the system expects it during the match phase!
